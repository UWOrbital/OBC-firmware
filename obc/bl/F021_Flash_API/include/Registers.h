...
#define WIDTH_MAIN_BANK \
  ((uint8_t)(((uint32_t)FLASH_CONTROL_REGISTER->FcfgBank.FCFG_BANK_BITS.MAIN_BANK_WIDTH & (uint32_t)0x1C0U) >> 3U))
/* Macro to get the width of the ECC on Main Banks */
#define WIDTH_MAIN_ECC \
  ((uint8_t)(((uint32_t)FLASH_CONTROL_REGISTER->FcfgBank.FCFG_BANK_BITS.MAIN_BANK_WIDTH & (uint32_t)0x038U) >> 3U))
/* Macro to get the width of the Data on EEPROM Bank */
#define WIDTH_EEPROM_BANK \
  ((uint8_t)(((uint32_t)FLASH_CONTROL_REGISTER->FcfgBank.FCFG_BANK_BITS.EE_BANK_WIDTH & (uint32_t)0x0C0U) >> 3U))
/* Macro to get the width of the ECC on EEPROM Bank */
#define WIDTH_EEPROM_ECC \
  ((uint8_t)(((uint32_t)FLASH_CONTROL_REGISTER->FcfgBank.FCFG_BANK_BITS.EE_BANK_WIDTH & (uint32_t)0x018U) >> 3U))
#endif
