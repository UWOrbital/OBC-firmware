  ...
  */
  for (ulTries = 0U; ulTries < 5U; ulTries++) {
    ...
    */
    ...
    /*
      vTaskDelay workaround for baremetal no-OS, delay for 500ms
      rough estimate of cycles per iteration
    */
    uint8_t cyclesPerIteration = 8;  // Rough estimate of cycles per iteration
    unsigned long cycles = (500 * (CLOCK_SPEED_HZ / 1000)) / cyclesPerIteration;
...
static REDSTATUS DiskRead(uint8_t bVolNum, uint64_t ullSectorStart, uint32_t ulSectorCount, void *pBuffer) {
  ...
  while (ulSectorIdx < ulSectorCount) {
    ...
    result = disk_read(bVolNum, &pbBuffer[ulSectorIdx * ulSectorSize], (uint32_t)(ullSectorStart + ulSectorIdx),
                       (uint8_t)ulTransfer);
    if (result != RES_OK) {
      ret = -RED_EIO;
      break;
...
static REDSTATUS DiskWrite(uint8_t bVolNum, uint64_t ullSectorStart, uint32_t ulSectorCount, const void *pBuffer) {
  ...
  while (ulSectorIdx < ulSectorCount) {
    ...
    result = disk_write(bVolNum, &pbBuffer[ulSectorIdx * ulSectorSize], (uint32_t)(ullSectorStart + ulSectorIdx),
                        (uint8_t)ulTransfer);
    if (result != RES_OK) {
      ret = -RED_EIO;
      break;
