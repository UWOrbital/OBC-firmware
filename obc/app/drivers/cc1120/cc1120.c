...
// see p.104 for Random Number Generator register config
 ...
 */
obc_error_code_t
cc1120ReadSpi(uint8_t addr, uint8_t data[], uint8_t len) {
 ...
 *
 * @param canary_value- A pointer to an 8-bit integer to store the stack canary in
 * @param receiver_data- A pointer to an 8-bit integer to store the receiver noise in
 * @return obc_error_code_t - Whether or not the register read was successful
 */
obc_error_code_t cc1120Rng(uint8_t *canary_value, uint8_t *receiver_data) {
  // change to rx mode
  obc_error_code_t errCode;
  RETURN_IF_ERROR_CODE(cc1120StrobeSpi(CC1120_STROBE_SRX));
  RETURN_IF_ERROR_CODE(cc1120ReadExtAddrSpi(CC1120_REGS_EXT_RNDGEN, canary_value, 1));
  RETURN_IF_ERROR_CODE(cc1120ReadFifo(receiver_data, 1));
  (*canary_value) &= ~(1 << 7);
  (*canary_value) ^= *receiver_data;
  return OBC_ERR_CODE_SUCCESS;
}
