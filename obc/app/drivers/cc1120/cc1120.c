 ...
 *
 * @param canarayValue- A pointer to an 8-bit integer to store the stack canary in
 * @param receiverData- A pointer to an 8-bit integer to store the receiver noise in
 * @return obc_error_code_t - Whether or not the register read was successful
 */
obc_error_code_t cc1120Rng(uint8_t *canaryValue, uint8_t *receiverData) {
  // change to rx mode
  obc_error_code_t errCode;
  RETURN_IF_ERROR_CODE(cc1120StrobeSpi(CC1120_STROBE_SRX));
  RETURN_IF_ERROR_CODE(cc1120ReadExtAddrSpi(CC1120_REGS_EXT_RNDGEN, canaryValue, 1));
  RETURN_IF_ERROR_CODE(cc1120ReadFifo(receiverData, 1));
  (*canaryValue) &= ~(1 << 7);
  (*canaryValue) ^= *receiverData;
  return OBC_ERR_CODE_SUCCESS;
}
