 ...
 */
#define RED_O_MASK                                                                                                     \
  (RED_O_RDONLY | RED_O_WRONLY | RED_O_RDWR | RED_O_APPEND | RED_O_CREAT | RED_O_EXCL | RED_O_TRUNC | RED_O_NOFOLLOW | \
   RED_O_SYMLINK_IF_ENABLED)
...
#endif
static REDSTATUS PathStartingPoint(int32_t iDirFildes, const char *pszPath, uint8_t *pbVolNum, uint32_t *pulDirInode,
                                   const char **ppszLocalPath);
static REDSTATUS FildesOpen(int32_t iDirFildes, const char *pszPath, uint32_t ulOpenMode, FTYPE type, uint16_t uMode,
                            int32_t *piFildes);
...
/** @brief Mount a file system volume.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_mount(const char *pszVolume) { return red_mount2(pszVolume, RED_MOUNT_DEFAULT); }
...
/** @brief Unmount a file system volume.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_umount(const char *pszVolume) { return red_umount2(pszVolume, RED_UMOUNT_DEFAULT); }
...
/** @brief Format a file system volume.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_format(const char *pszVolume) { return red_format2(pszVolume, NULL); }
...
/** @brief Free inodes orphaned before the most recent mount.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_freeorphans(const char *pszVolume, uint32_t ulCount) {
  REDSTATUS err;
  int32_t ret;
...
/** @brief Open a file or directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_open(const char *pszPath, uint32_t ulOpenFlags) {
      ...
      if (ret == 0) {
        ...
        */
        if (ret == 0) {
          ...
          */
          if ((ulLenRead < ulBufferSize) && ((ulLenRead == 0U) || (pszBuffer[ulLenRead - 1U] != '\0'))) {
...
/** @brief Delete a file or directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_unlink(const char *pszPath) { return red_unlinkat(RED_AT_FDNONE, pszPath, 0U); }
...
/** @brief Change the mode of a file or directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_chmod(const char *pszPath, uint16_t uMode) { return red_fchmodat(RED_AT_FDNONE, pszPath, uMode, 0U); }
...
/** @brief Create a new directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_mkdir(const char *pszPath) { return red_mkdirat(RED_AT_FDNONE, pszPath, RED_S_IDIR_DEFAULT); }
...
/** @brief Create a new directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_mkdir2(const char *pszPath, uint16_t uMode) { return red_mkdirat(RED_AT_FDNONE, pszPath, uMode); }
...
/** @brief Delete a directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_rmdir(const char *pszPath) { return red_unlinkat(RED_AT_FDNONE, pszPath, RED_AT_REMOVEDIR); }
#endif
...
/** @brief Create a hard link, optionally via paths which are relative to given
    ...
    - #RED_EXDEV: @p pszPath and @p pszHardLink are on different file system
      volumes.
*/
int32_t red_linkat(int32_t iDirFildes, const char *pszPath, int32_t iHardLinkDirFildes, const char *pszHardLink,
                   uint32_t ulFlags) {
  REDSTATUS ret;
...
/** @brief Get the status of a file or directory.
    ...
    - #RED_EUSERS: Cannot become a file system user: too many users.
*/
int32_t red_stat(const char *pszPath, REDSTAT *pStat) { return red_fstatat(RED_AT_FDNONE, pszPath, pStat, 0U); }
...
REDDIRENT *red_readdir(REDDIR *pDirStream) {
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ret = RedCoreDirRead(pDirStream->pOpenIno->ulInode, &pDirStream->o.ulDirPosition, pDirStream->dirent.d_name,
...
*/
void red_rewinddir(REDDIR *pDirStream) { red_seekdir(pDirStream, 0U); }
...
static int32_t ReadSub(int32_t iFildes, void *pBuffer, uint32_t ulLength, bool fIsPread, uint64_t ullOffset) {
  ...
  if (ret == 0) {
    if (ulLength > (uint32_t)INT32_MAX) {
      ...
      if (ret == 0) {
        ulLenRead = ulLength;
        ret = RedCoreFileRead(pHandle->pOpenIno->ulInode, fIsPread ? ullOffset : pHandle->o.ullFileOffset, &ulLenRead,
                              pBuffer);
      }
    ...
    if (iDirFildes != RED_AT_FDABS) {
      ...
      */
      if (((ret == -RED_ENOENT) || ((ret == 0) && (gpRedVolConf->pszPathPrefix[0U] == '\0'))) &&
          (pszPath[0U] != REDCONF_PATH_SEPARATOR) && (pszPath[0U] != '\0')) {
...
*/
static REDSTATUS FildesOpen(int32_t iDirFildes, const char *pszPath, uint32_t ulOpenFlags, FTYPE type, uint16_t uMode,
                            int32_t *piFildes) {
  uint32_t ulDirInode;
  const char *pszLocalPath;
  REDSTATUS ret;
...
#if REDCONF_READ_ONLY == 0
    else if (gpRedVolume->fReadOnly && ((ulOpenFlags & (RED_O_WRONLY | RED_O_RDWR | RED_O_TRUNC)) != 0U)) {
      ...
      if (pHandle == NULL) {
        ...
        if ((ulOpenFlags & RED_O_CREAT) != 0U) {
          ...
          if (ret == 0) {
            ...
            */
            else if ((ret == -RED_EROFS) || ((ret == -RED_EEXIST) && ((ulOpenFlags & RED_O_EXCL) == 0U))) {
...
static REDSTATUS FildesToHandle(int32_t iFildes, FTYPE expectedType, REDHANDLE **ppHandle) {
  ...
  if (ppHandle == NULL) {
    ...
    if ((uHandleIdx >= REDCONF_HANDLE_COUNT) || (bVolNum >= REDCONF_VOLUME_COUNT) ||
        (gaHandle[uHandleIdx].pOpenIno == NULL) || (gaHandle[uHandleIdx].pOpenIno->bVolNum != bVolNum) ||
...
static bool DirStreamIsValid(const REDDIR *pDirStream) {
  ...
  if (!HANDLE_PTR_IS_VALID(pDirStream)) {
     ...
     */
    fRet = false;
  } else if ((pDirStream->pOpenIno == NULL) || (pDirStream->pOpenIno->bVolNum >= REDCONF_VOLUME_COUNT) ||
...
*/
static REDSTATUS OpenInoDeref(OPENINODE *pOpenIno, bool fDoCleanup, bool fPropagateCleanupError) {
  REDSTATUS ret = 0;
