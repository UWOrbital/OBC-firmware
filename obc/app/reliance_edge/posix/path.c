...
static REDSTATUS PathWalk(uint32_t ulDirInode, const char *pszLocalPath, uint32_t ulFlags, REDSTATUS rootDirError,
                          uint32_t *pulPInode, const char **ppszName, uint32_t *pulInode);
...
/** @brief Given a path, return the parent inode number and a pointer to the
    ...
    @retval -RED_ENAMETOOLONG   The path (excluding the final path component)
                                ...
                                longer than #REDCONF_NAME_MAX.
*/
REDSTATUS RedPathToName(uint32_t ulDirInode, const char *pszLocalPath, REDSTATUS rootDirError, uint32_t *pulPInode,
                        const char **ppszName) {
  REDSTATUS ret;
...
static REDSTATUS PathWalk(uint32_t ulDirInode, const char *pszLocalPath, uint32_t ulFlags, REDSTATUS rootDirError,
  ...
  if ((ulDirInode == INODE_INVALID) || (pszLocalPath == NULL) || ((ulFlags & RED_AT_SYMLINK_NOFOLLOW) != ulFlags) ||
...
static REDSTATUS SymlinkNext(PATHWALKCTX *pCtx) {
  ...
  */
  if ((pCtx == NULL) || (pCtx->pszPath == NULL) || (pCtx->pszName != NULL)) {
    ...
    */
    while ((ret == 0) && (pSymlinkCtx->bIdx != SYMLINK_IDX_NONE) && (pCtx->pszName == NULL)) {
          ...
          if ((ulReadLen > 0U) && (pSymlinkCtx->szName[0U] == REDCONF_PATH_SEPARATOR)) {
            ...
            */
            ...
            /*  Check if we have a complete name after skipping over
                ...
                data that we read.
            */
            if ((ulReadLen == ulReadMax) && ((ulNameIdx + RedNameLen(&pSymlinkCtx->szName[ulNameIdx])) == ulReadLen)) {
...
static bool IsDot(const char *pszPathComponent) {
  /*  Match "." or "./"
   */
  return (pszPathComponent != NULL) && (pszPathComponent[0U] == '.') &&
...
static bool IsDotDot(const char *pszPathComponent) {
  /*  Match ".." or "../"
   */
  return (pszPathComponent != NULL) && (pszPathComponent[0U] == '.') && (pszPathComponent[1U] == '.') &&
