...
REDSTATUS RedVolFormat(const REDFMTOPT *pOptions) {
  ...
  */
  ...
  if (ret == 0) {
    ...
    ret = RedBufferGet(BLOCK_NUM_MASTER, (uint16_t)((uint32_t)BFLAG_META_MASTER | BFLAG_NEW | BFLAG_DIRTY),
                       (void **)&pMB);
    if (ret == 0) {
...
static uint32_t ComputeInodeCount(void) {
   ...
   */
  ulInodeBlocksMax = (uint32_t)REDMIN(
      UINT32_MAX, UINT64_SUFFIX(2) + INODE_DATA_BLOCKS + REDCONF_INDIRECT_POINTERS + (DINDIR_POINTERS * INDIR_ENTRIES));
