...
REDSTATUS RedVolInitBlockGeometry(void) {
  ...
  if ((pBdevInfo->ulSectorSize < SECTOR_SIZE_MIN) || ((REDCONF_BLOCK_SIZE % pBdevInfo->ulSectorSize) != 0U) ||
      ((UINT64_MAX - gpRedVolConf->ullSectorOffset) <
       pBdevInfo->ullSectorCount)) /* SectorOffset + SectorCount must not wrap */
  {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
  ...
  if (ulFlags != (ulFlags & RED_MOUNT_MASK)) {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        ...
        REDASSERT((gpRedMR->ulOrphanHead == INODE_INVALID) == (gpRedMR->ulOrphanTail == INODE_INVALID));
      }
#endif
  ...
  */
  ...
  if (ret == 0) {
    /*  Verify that the driver was compiled with the same settings that
        ...
        to be reformatted.
    */
    if (!RED_DISK_LAYOUT_IS_SUPPORTED(pMB->ulVersion) || (pMB->ulBlockCount > gpRedVolume->ulBlockCount) ||
        (pMB->uMaxNameLen != REDCONF_NAME_MAX) || (pMB->uDirectPointers != REDCONF_DIRECT_POINTERS) ||
        (pMB->uIndirectPointers != REDCONF_INDIRECT_POINTERS) || (pMB->bBlockSizeP2 != BLOCK_SIZE_P2) ||
        ...
        (((pMB->bFlags & MBFLAG_INODE_UIDGID) != 0U) !=
         ((REDCONF_API_POSIX == 1) && (REDCONF_POSIX_OWNER_PERM == 1))) ||
