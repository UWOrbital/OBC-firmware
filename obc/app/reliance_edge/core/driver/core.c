...
*/
REDSTATUS RedCoreVolFormat(const REDFMTOPT *pOptions) { return RedVolFormat(pOptions); }
...
*/
REDSTATUS RedCoreVolMount(uint32_t ulFlags) { return RedVolMount(ulFlags); }
...
#endif /* (REDCONF_READ_ONLY == 0) && (REDCONF_API_POSIX == 1) && ((REDCONF_API_POSIX_UNLINK == 1) || \
