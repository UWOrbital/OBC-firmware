...
#define MBFEATURE_MASK_WRITEABLE \
  (((REDCONF_API_POSIX == 1) && (REDCONF_API_POSIX_SYMLINK == 1)) ? MBFEATURE_SYMLINK : 0U)
...
typedef struct {
  ...
  uint16_t uFeaturesIncompat; /**< Feature booleans which affect on-disk structures.  Must match features supported by
                                 the driver in order to mount. */
  uint16_t uFeaturesReadOnly; /**< Feature booleans which affect on-disk structures.  Must match features supported by
                                 the driver in order to mount read/write. */
  uint8_t bSectorSizeP2; /**< Size of a sector, expressed as a power of two, used to generate METAROOT::ulSectorCRC. */
} MASTERBLOCK;

#define METAROOT_HEADER_SIZE \
  (NODEHEADER_SIZE + 12U + ((REDCONF_API_POSIX == 0) ? 0U : (4U + ((REDCONF_DELETE_OPEN == 1) ? 12U : 0U))))
#define METAROOT_ENTRY_BYTES \
  (REDCONF_BLOCK_SIZE - METAROOT_HEADER_SIZE) /* Number of bytes remaining in the metaroot block for entries. */
...
typedef struct {
  ...
  uint32_t ulOrphanTail; /**< Tail of the list of orphaned inodes.  Enables concatenation of the lists during mount in
                            O(1) time. */
#endif
...
#define IMAPNODE_ENTRY_BYTES \
  (REDCONF_BLOCK_SIZE - IMAPNODE_HEADER_SIZE) /* Number of bytes remaining in the imap node for entries. */
...
#define INODE_HEADER_SIZE                                                                                          \
  (NODEHEADER_SIZE + 8U + ((REDCONF_INODE_BLOCKS == 1) ? 4U : 0U) + ((REDCONF_INODE_TIMESTAMPS == 1) ? 12U : 0U) + \
   4U + POSIX_INODE_HEADER_SIZE)
