...
REDSTATUS RedPermCheck(uint8_t bAccess, uint16_t uMode, uint32_t ulUid, uint32_t ulGid) {
  ...
  if (!RedOsIsPrivileged()) {
    ...
    if ((((bAccess & RED_X_OK) != 0U) &&
         (fUib ? ((uMode & RED_S_IXUSR) == 0U)
               : (fGib ? ((uMode & RED_S_IXGRP) == 0U) : ((uMode & RED_S_IXOTH) == 0U)))) ||
        (((bAccess & RED_W_OK) != 0U) &&
         (fUib ? ((uMode & RED_S_IWUSR) == 0U)
               : (fGib ? ((uMode & RED_S_IWGRP) == 0U) : ((uMode & RED_S_IWOTH) == 0U)))) ||
        (((bAccess & RED_R_OK) != 0U) &&
         (fUib ? ((uMode & RED_S_IRUSR) == 0U)
               : (fGib ? ((uMode & RED_S_IRGRP) == 0U) : ((uMode & RED_S_IROTH) == 0U))))) {
      ret = -RED_EACCES;
    }
  }
...
REDSTATUS RedPermCheckUnlink(uint16_t uPMode, uint32_t ulPUid, uint32_t ulPGid, uint32_t ulFUid) {
  ...
  if (!RedOsIsPrivileged()) {
    ...
    if ((ret == 0) && ((uPMode & RED_S_ISVTX) != 0U) && (RedOsUserId() != ulFUid) && (RedOsUserId() != ulPUid)) {
      ret = -RED_EACCES;
    }
  }
#endif
