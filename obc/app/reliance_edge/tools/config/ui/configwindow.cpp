...
ConfigWindow::ConfigWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::ConfigWindow) {
  ...
  allSettings.cbsAutomaticDiscards = new CbSetting(macroNameAutomaticDiscards, false, validateAutomaticDiscards,
                                                   ui->cbAutomaticDiscards, ui->wbtnAutomaticDiscards);
  allSettings.rbtnsUsePosix =
      new RbtnSetting(macroNameUsePosix, true, validateUsePosixApi, ui->rbtnUsePosix, ui->wbtnApiRbtns);
  allSettings.rbtnsUseFse =
      new RbtnSetting(macroNameUseFse, false, validateUseFseApi, ui->rbtnUseFse, ui->wbtnApiRbtns);
  ...
  allSettings.cbsPosixAtomicRename =
      new CbSetting(macroNamePosixRenameAtomic, true, emptyBoolValidator, ui->cbPosixAtomicRename);
  allSettings.cbsPosixFtruncate =
      new CbSetting(macroNamePosixFtruncate, true, emptyBoolValidator, ui->cbPosixFtruncate);
  ...
  allSettings.cbsPosixFstrim =
      new CbSetting(macroNamePosixFstrim, false, validatePosixFstrim, ui->cbPosixFstrim, ui->wbtnFstrim);
  allSettings.cbsPosixOwnerPerm =
      new CbSetting(macroNamePosixOwnerPerm, false, emptyBoolValidator, ui->cbPosixOwnerPerm);
  allSettings.sbsMaxNameLen =
      new SbSetting(macroNameMaxNameLen, 12, validateMaxNameLen, ui->sbFileNameLen, ui->wbtnFileNameLen);
  allSettings.pssPathSepChar = new PathSepSetting(macroNamePathSepChar, "/", validatePathSepChar, ui->cmbPathChar,
                                                  ui->lePathCharCustom, ui->wbtnPathChar);
  ...
  allSettings.sbsTaskCount =
      new SbSetting(macroNameTaskCount, 10, validateTaskCount, ui->sbTaskCount, ui->wbtnTaskCount);
  allSettings.sbsHandleCount =
      new SbSetting(macroNameHandleCount, 10, validateHandleCount, ui->sbHandleCount, ui->wbtnHandleCount);
  allSettings.cbsDebugEnableOutput =
      new CbSetting(macroNameDebugEnableOutput, true, emptyBoolValidator, ui->cbEnableOutput);
  allSettings.cbsDebugProcesAsserts =
      new CbSetting(macroNameDebugProcesAsserts, false, emptyBoolValidator, ui->cbProcessAsserts);

  // "Volumes" tab (note: most settings handled by VolumeSettings)
  allSettings.cmisBlockSize =
      new CmbIntSetting(macroNameBlockSize, 512, validateBlockSize, ui->cmbBlockSize, ui->wbtnBlockSize);

  // "Data Storage" tab
  allSettings.cmssByteOrder =
      new CmbStrSetting(macroNameByteOrder, "Little endian", validateByteOrder, ui->cmbByteOrder, ui->wbtnByteOrder);
  allSettings.cmisNativeAlignment = new CmbIntSetting(macroNameNativeAlignment, 4, validateAlignmentSize,
                                                      ui->cmbAlignmentSize, ui->wbtnAlignmentSize);
  allSettings.cmssCrc =
      new CmbStrSetting(macroNameCrc, "Slice by 8 - largest, fastest", validateCrc, ui->cmbCrc, ui->wbtnCrc);
  allSettings.cbsInodeBlockCount =
      new CbSetting(macroNameInodeCount, true, validateInodeBlockCount, ui->cbInodeBlockCount, ui->wbtnInodeBlockCount);
  allSettings.cbsInodeTimestamps = new CbSetting(macroNameInodeTimestamps, true, validateInodeTimestamps,
                                                 ui->cbInodeTimestamps, ui->wbtnInodeTimestamps);
  allSettings.cbsUpdateAtime = new CbSetting(macroNameUpdateAtime, false, emptyBoolValidator, ui->cbUpdateAtime);
  allSettings.sbsDirectPtrs =
      new SbSetting(macroNameDirectPtrs, 4, validateDirectPointers, ui->sbDirectPointers, ui->wbtnDirectPointers);
  allSettings.sbsIndirectPtrs = new SbSetting(macroNameIndirectPtrs, 32, validateIndirectPointers,
                                              ui->sbIndirectPointers, ui->wbtnIndirectPointers);

  // "Memory" tab
  allSettings.sbsAllocatedBuffers = new SbSetting(macroNameAllocatedBuffers, 12, validateAllocatedBuffers,
                                                  ui->sbAllocatedBuffers, ui->wbtnAllocatedBuffers);
  allSettings.cmisBufferAlignment = new CmbIntSetting(macroNameBufferAlignment, 8, validateBufferAlignment,
                                                      ui->cmbBufferAlignment, ui->wbtnBufferAlignment);
  allSettings.sbsBufferWriteGatherKb =
      new CheckedSbSetting(macroNameBufferGatherSize, 1, validateBufferWriteGather, ui->sbBufferWriteGatherKB,
                           ui->cbBufferWriteGather, false, true, ui->wbtnBufferWriteGather);
  ...
  allSettings.cbsTrManual =
      new CbSetting(macroNameTrManual, false, validateTransactManual, ui->cbTransactManual, ui->wbtnTransactManual);
  ...
  allSettings.cbsTrVolFull =
      new CbSetting(macroNameTrVolFull, true, validateTransactVolFull, ui->cbTransactVolFull, ui->wbtnTransactVolFull);
  ...
  volumeSettings = new VolumeSettings(
      ui->lePathPrefix, ui->cmbSectorSize, ui->cbSectorSizeAuto, ui->sbVolSize, ui->cbVolSizeAuto,
      ui->labelVolSizeBytes, ui->sbVolOff, ui->labelVolOffBytes, ui->sbInodeCount, ui->cbInodeCountAuto,
      ui->cmbAtomicWrite, ui->cmbDiscardsSupported, ui->cbEnableRetries, ui->sbBlockIoRetries, ui->widgetBlockIoRetries,
      ui->btnAddVol, ui->btnRemoveCurrVol, ui->listVolumes, ui->wbtnVolumeCtrls, ui->wbtnPathPrefix, ui->wbtnSectorSize,
      ui->wbtnVolSize, ui->wbtnVolOff, ui->wbtnInodeCount, ui->wbtnAtomicWrite, ui->wbtnDiscardsSupported,
      ui->wbtnIoRetries);
  ...
  for (int i = 0; i < wbtns.count(); i++) {
    connect(wbtns[i], SIGNAL(clicked()), this, SIGNAL(warningBtnClicked()));
  }
  connect(ui->cbReadonly, SIGNAL(toggled(bool)), this, SLOT(cbReadonly_toggled(bool)));
  connect(ui->cbAutomaticDiscards, SIGNAL(toggled(bool)), this, SLOT(cbAutomaticDiscards_toggled(bool)));
  connect(ui->rbtnUsePosix, SIGNAL(toggled(bool)), this, SLOT(rbtnUsePosix_toggled(bool)));
  connect(ui->cbPosixRename, SIGNAL(toggled(bool)), this, SLOT(cbPosixRename_toggled(bool)));
  connect(ui->cbPosixMkdir, SIGNAL(toggled(bool)), this, SLOT(cbPosixMkdir_toggled(bool)));
  connect(ui->cbPosixLink, SIGNAL(toggled(bool)), this, SLOT(cbPosixLink_toggled(bool)));
  connect(ui->cbPosixUnlink, SIGNAL(toggled(bool)), this, SLOT(cbPosixUnlink_toggled(bool)));
  connect(ui->cbPosixFtruncate, SIGNAL(toggled(bool)), this, SLOT(cbPosixFtruncate_toggled(bool)));
  connect(ui->cbPosixFstrim, SIGNAL(toggled(bool)), this, SLOT(cbPosixFstrim_toggled(bool)));
  connect(ui->cbFseTruncate, SIGNAL(toggled(bool)), this, SLOT(cbFseTruncate_toggled(bool)));
  connect(ui->cbInodeTimestamps, SIGNAL(toggled(bool)), this, SLOT(cbInodeTimestamps_toggled(bool)));
  connect(ui->rbtnMemUseCStd, SIGNAL(toggled(bool)), this, SLOT(rbtnMemUseCStd_toggled(bool)));
  connect(ui->rbtnMemUseReliance, SIGNAL(toggled(bool)), this, SLOT(rbtnMemUseReliance_toggled(bool)));
  connect(ui->rbtnMemCustomize, SIGNAL(toggled(bool)), this, SLOT(rbtnMemCustomize_toggled(bool)));
  connect(ui->cbTransactManual, SIGNAL(toggled(bool)), this, SLOT(cbTransactManual_toggled(bool)));
  connect(ui->actionAbout, SIGNAL(triggered()), this, SLOT(actionAbout_clicked()));

  // Forwarded signals
  connect(ui->actionSave, SIGNAL(triggered()), this, SIGNAL(saveClicked()));
  connect(ui->actionSave_As, SIGNAL(triggered()), this, SIGNAL(saveAsClicked()));
  connect(ui->actionLoad, SIGNAL(triggered()), this, SIGNAL(loadClicked()));
...
void ConfigWindow::rbtnUsePosix_toggled(bool selected) {
  ...
  ui->cbTransactTruncate->setEnabled((selected && ui->cbPosixFtruncate->isChecked()) ||
...
void ConfigWindow::cbPosixMkdir_toggled(bool selected) { ui->cbTransactDirCreate->setEnabled(selected); }

void ConfigWindow::cbPosixLink_toggled(bool selected) { ui->cbTransactLink->setEnabled(selected); }

void ConfigWindow::cbPosixUnlink_toggled(bool selected) { ui->cbTransactUnlink->setEnabled(selected); }
...
void ConfigWindow::cbTransactManual_toggled(bool selected) { ui->frameAutomaticTransactions->setEnabled(!selected); }
