...
static QString outputIfNotBlank(const QString &macroName, const QString &value, const QString &comment = QString());
static qint32 getMinCompatVer();
template <typename T>
static void parseToSetting(const QString &text, Setting<T> *setting, QStringList &notFound, QStringList &notParsed,
                           const QString &humanName = QString());
static void parseMemSetting(const QString &text, StrSetting *setting);
static void parseToEnabledDisabledSetting(const QString &text, StrSetting *setting, const QString &strTrue,
                                          const QString &strFalse, QStringList &notFound, QStringList &notParsed,
                                          const QString &humanName = QString());
...
const QString str1 = QString("1"), str0 = QString("0"), strU = QString("U");
...
static inline void addBoolSetting(QString &outputString, BoolSetting *boolSetting) {
  outputString += outputLine(boolSetting->GetMacroName(), (boolSetting->GetValue() ? str1 : str0));
}

static inline void addIntSetting(QString &outputString, IntSetting *intSetting) {
  outputString += outputLine(intSetting->GetMacroName(), QString::number(intSetting->GetValue()) + strU);
}
...
static inline void addTrIfChecked(QString &outputString, BoolSetting *trSetting, bool addAnyway = true) {
...
QString AllSettings::FormatHeaderOutput() {
  ...
  toReturn += outputLine(
      allSettings.cbsPosixAtomicRename->GetMacroName(),
      (posix && allSettings.cbsPosixRename->GetValue() && allSettings.cbsPosixAtomicRename->GetValue() ? str1 : str0));
  ...
  toReturn +=
      outputLine(allSettings.cbsPosixCwd->GetMacroName(), (posix && allSettings.cbsPosixCwd->GetValue() ? str1 : str0));
  ...
  currValue = (allSettings.cmssByteOrder->GetValue().startsWith("big", Qt::CaseInsensitive) ? "1" : "0");
  toReturn += outputLine(macroNameByteOrder, currValue);

  addIntSetting(toReturn, allSettings.cmisNativeAlignment);

  currValue =
      (allSettings.cmssCrc->GetValue().startsWith("bitwise", Qt::CaseInsensitive)
           ? crcBitwise
           : (allSettings.cmssCrc->GetValue().startsWith("sarwate", Qt::CaseInsensitive) ? crcSarwate : crcSlice));
  ...
  toReturn += outputLine(
      allSettings.cbsUpdateAtime->GetMacroName(),
      ((allSettings.cbsInodeTimestamps->GetValue() && allSettings.cbsUpdateAtime->GetValue()) ? str1 : str0));
  ...
  if (allSettings.cbsTrManual->GetValue()) {
    ...
    addTrIfChecked(currValue, allSettings.cbsTrDirCreat, posix && allSettings.cbsPosixMkdir->GetValue());
    addTrIfChecked(currValue, allSettings.cbsTrRename, posix && allSettings.cbsPosixRename->GetValue());
    addTrIfChecked(currValue, allSettings.cbsTrLink, posix && allSettings.cbsPosixLink->GetValue());
    addTrIfChecked(currValue, allSettings.cbsTrUnlink, posix && allSettings.cbsPosixUnlink->GetValue());
    addTrIfChecked(currValue, allSettings.cbsTrWrite);
    addTrIfChecked(
        currValue, allSettings.cbsTrTruncate,
        (posix && allSettings.cbsPosixFtruncate->GetValue()) || (!posix && allSettings.cbsFseTruncate->GetValue()));
  ...
  toReturn += QString("#define ") + macroNameInlineImap + (imapInline ? QString(" 1\n\n") : QString(" 0\n\n")) +
  ...
  toReturn +=
      QString("#define RED_CONFIG_UTILITY_VERSION 0x") + QString::number(CONFIG_VERSION_VAL, 16) + QString("U\n\n");

  toReturn +=
      QString("#define RED_CONFIG_MINCOMPAT_VER 0x") + QString::number(getMinCompatVer(), 16) + QString("U\n\n");
...
// based on them
QString outputLine(const QString &macroName, const QString &value, const QString &comment) {
  return QString("#define ") + macroName + QString(" ") + value +
         (comment.isNull() ? QString("") : QString(" /* ") + comment + QString(" */")) + QString("\n\n");
}

// Wraps outputLine, but returns an empty string if the given
// string value is null, empty, or whitespace.
QString outputIfNotBlank(const QString &macroName, const QString &value, const QString &comment) {
...
void AllSettings::CheckError(SettingBase *setting, QStringList &errors, QStringList &warnings) {
...
void AllSettings::ParseHeaderToSettings(const QString &text, QStringList &notFound, QStringList &notParsed) {
  ...
  // "Data Storage" tab
  parseToEnabledDisabledSetting(text, allSettings.cmssByteOrder, "Big endian", "Little endian", notFound, notParsed);
...
// specified.
template <typename T>
void parseToSetting(const QString &text, Setting<T> *setting, QStringList &notFound, QStringList &notParsed,
...
void parseToEnabledDisabledSetting(const QString &text, StrSetting *setting, const QString &strTrue,
                                   const QString &strFalse, QStringList &notFound, QStringList &notParsed,
                                   const QString &humanName) {
  QString label = (humanName.isNull() ? setting->GetMacroName() : humanName);
  bool found = false;
  ...
  QString strValue = findValue(text, setting->GetMacroName(), found);
  if (found && !strValue.isNull()) {
    if (QString::compare(strValue, "0") == 0 || QString::compare(strValue, "false", Qt::CaseInsensitive) == 0) {
      setting->SetValue(strFalse);
    } else if (QString::compare(strValue, "1") == 0 || QString::compare(strValue, "true", Qt::CaseInsensitive) == 0) {
...
void AllSettings::ParseCodefileToSettings(const QString &text, QStringList &notFound, QStringList &notParsed) {
  Q_ASSERT(volumeSettings != NULL);
