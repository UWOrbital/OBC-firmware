...
struct AllSettings {
  ...
  ///
  static void ParseHeaderToSettings(const QString &text, QStringList &notFound, QStringList &notParsed);

  ///
  /// \brief  Wrapper function for VolumeSettings::ParseCodefile
  ///
  static void ParseCodefileToSettings(const QString &text, QStringList &notFound, QStringList &notParsed);

  ///
  /// \brief  Checks for validity of \p setting, appending any message to
  ///         \p errors or \p warnings.
  ///
  static void CheckError(SettingBase *setting, QStringList &errors, QStringList &warnings);
