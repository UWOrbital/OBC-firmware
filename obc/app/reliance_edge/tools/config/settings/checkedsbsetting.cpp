...
CheckedSbSetting::CheckedSbSetting(QString macroName, unsigned long defaultValue,
                                   ...
                                   QCheckBox *cb, bool isChecked, bool enableWhenChecked, WarningBtn *btnWarn)
    : IntSetting(macroName, defaultValue, validator, btnWarn), spinbox(sb), checkBox(cb) {
  if (!sb) {
 ...
 */
void CheckedSbSetting::spinbox_valueChanged(const QString &text) { ProcessInput(text); }
