...
static int WriteToFile(uint8_t bVolNum, const FILEMAPPING *pFileMapping, uint64_t ullOffset, const void *pData,
                       uint32_t ulDataLen);
...
*/
static void FreeStrList(STRLISTENTRY **ppStrList) {
...
*/
int IbFseGetFileList(const char *pszMapPath, const char *pszIndirPath, FILELISTENTRY **ppFileListHead) {
  int ret = 0;
...
int IbCopyFile(uint8_t bVolNum, const FILEMAPPING *pFileMapping) {
  ...
  while ((ret == 0) && (ullCurrOffset < ullFSize)) {
    ...
    ulCurrLen =
        ((ullFSize - ullCurrOffset) < gulCopyBufferSize) ? (uint32_t)(ullFSize - ullCurrOffset) : gulCopyBufferSize;
...
/** @brief Output C macros for the file list.
    ...
    @retval -1  An error occurred.
*/
int IbFseOutputDefines(FILELISTENTRY *pFileList, const IMGBLDPARAM *pParam) {
  int ret = 0;
