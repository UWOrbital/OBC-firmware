...
typedef enum { DOWNLINK_TELEMETRY_FILE, DOWNLINK_DATA_BUFFER, DOWNLINK_CMD_RESPONSE, DOWNLINK_IMAGE } encode_event_id_t;
...
typedef struct {
  ...
  union {
    ...
    uint8_t cameraId;  // DOWNLINK_IMAGE: which camera's FIFO to stream (camera_id_t)
  };
} encode_event_t;
