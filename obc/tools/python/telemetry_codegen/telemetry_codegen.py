...
def generate_c_macros(messages):
    ...
    for message in messages:
        if (reg.match(message['name']) == None):
...
def generate_c_serialization_deserialization_functions(messages):
    ...
    for message in messages:
        ...
        for signal in message['signals']:
            serialize_func += (
                f"    bit_stream |= ((uint64_t){message['name']}_mailbox.{signal['name']} & {hex(2**signal['bit_width'] - 1)}) << bit_offset;\n"
