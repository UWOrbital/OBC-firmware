...
VolumeSettings::Volume::Volume(QString name, WarningBtn *wbtnPathPrefix, WarningBtn *wbtnSectorSize,
                               WarningBtn *wbtnVolSize, WarningBtn *wbtnVolOff, WarningBtn *wbtnInodeCount,
                               WarningBtn *wbtnAtomicWrite, WarningBtn *wbtnDiscardSupport,
...
StrSetting *VolumeSettings::Volume::GetStName() { return &stName; }

IntSetting *VolumeSettings::Volume::GetStSectorSize() { return &stSectorSize; }

IntSetting *VolumeSettings::Volume::GetStSectorCount() { return &stSectorCount; }

IntSetting *VolumeSettings::Volume::GetStSectorOff() { return &stSectorOff; }

IntSetting *VolumeSettings::Volume::GetStInodeCount() { return &stInodeCount; }

StrSetting *VolumeSettings::Volume::GetStAtomicWrite() { return &stAtomicWrite; }

StrSetting *VolumeSettings::Volume::GetStDiscardSupport() { return &stDiscardSupport; }

IntSetting *VolumeSettings::Volume::GetStBlockIoRetries() { return &stBlockIoRetries; }
...
bool VolumeSettings::Volume::NeedsExternalImap() {
  ...
  if (IsAutoSectorCount()) {
    ...
    unsigned long metarootHeaderSize = 16 + (allSettings.rbtnsUsePosix->GetValue() ? 16 : 12);
...
bool VolumeSettings::Volume::IsAutoSectorSize() { return fAutoSectorSize; }

bool VolumeSettings::Volume::IsAutoSectorCount() { return fAutoSectorCount; }

bool VolumeSettings::Volume::IsAutoInodeCount() { return fAutoInodeCount; }

void VolumeSettings::Volume::SetAutoSectorSize(bool value) {
  fAutoSectorSize = value;

  // Notify to update error/warning buttons.
  stSectorSize.Notify();
}
...
void VolumeSettings::Volume::SetAutoSectorCount(bool value) {
  ...
  // Notify to update error/warning buttons.
  stSectorCount.Notify();
  stInodeCount.Notify();
}

void VolumeSettings::Volume::SetAutoInodeCount(bool value) {
  fAutoInodeCount = value;

  // Notify to update error/warning buttons.
  stInodeCount.Notify();
}

VolumeSettings::VolumeSettings(QLineEdit *pathPrefixBox, QComboBox *sectorSizeBox, QCheckBox *sectorSizeAuto,
                               QSpinBox *volSizeBox, QCheckBox *volSizeAuto, QLabel *volSizeLabel, QSpinBox *volOffBox,
                               QLabel *volOffLabel, QSpinBox *inodeCountBox, QCheckBox *inodeCountAuto,
                               QComboBox *atomicWriteBox, QComboBox *discardSupportBox, QCheckBox *enableRetriesCheck,
                               QSpinBox *numRetriesBox, QWidget *numRetriesWidget, QPushButton *addButton,
                               QPushButton *removeButton, QListWidget *volumesList, WarningBtn *volCountWarn,
                               WarningBtn *pathPrefixWarn, WarningBtn *sectorSizeWarn, WarningBtn *volSizeWarn,
                               WarningBtn *volOffWarn, WarningBtn *inodeCountWarn, WarningBtn *atomicWriteWarn,
                               WarningBtn *discardSupportWarn, WarningBtn *ioRetriesWarn)
    : stVolumeCount(macroNameVolumeCount, 1, validateVolumeCount),
      ...
      cbInodeCountAuto(inodeCountAuto),
  ...
  connect(lePathPrefix, SIGNAL(textChanged(QString)), this, SLOT(lePathPrefix_textChanged(QString)));
  connect(sbVolSize, SIGNAL(valueChanged(QString)), this, SLOT(sbVolSize_valueChanged(QString)));
  connect(cbVolSizeAuto, SIGNAL(stateChanged(int)), this, SLOT(cbVolSizeAuto_stateChanged(int)));
  connect(sbVolOff, SIGNAL(valueChanged(QString)), this, SLOT(sbVolOff_valueChanged(QString)));
  connect(sbInodeCount, SIGNAL(valueChanged(QString)), this, SLOT(sbInodeCount_valueChanged(QString)));
  connect(cbInodeCountAuto, SIGNAL(stateChanged(int)), this, SLOT(cbInodeCountAuto_stateChanged(int)));
  connect(cmbSectorSize, SIGNAL(currentIndexChanged(int)), this, SLOT(cmbSectorSize_currentIndexChanged(int)));
  connect(cbSectorSizeAuto, SIGNAL(stateChanged(int)), this, SLOT(cbSectorSizeAuto_stateChanged(int)));
  connect(cmbAtomicWrite, SIGNAL(currentIndexChanged(int)), this, SLOT(cmbAtomicWrite_currentIndexChanged(int)));
  connect(cmbDiscardSupport, SIGNAL(currentIndexChanged(int)), this, SLOT(cmbDiscardSupport_currentIndexChanged(int)));
  connect(cbEnableRetries, SIGNAL(stateChanged(int)), this, SLOT(cbEnableRetries_stateChanged(int)));
  connect(sbNumRetries, SIGNAL(valueChanged(QString)), this, SLOT(sbNumRetries_valueChanged(QString)));
  connect(listVolumes, SIGNAL(currentRowChanged(int)), this, SLOT(listVolumes_currentRowChanged(int)));
  connect(btnAdd, SIGNAL(clicked()), this, SLOT(btnAdd_clicked()));
  connect(btnRemSelected, SIGNAL(clicked()), this, SLOT(btnRemSelected_clicked()));

  updateVolSizeBytes();
  updateVolOffBytes();
}

VolumeSettings::~VolumeSettings() { clearVolumes(); }

IntSetting *VolumeSettings::GetStVolumeCount() { return &stVolumeCount; }

QList<VolumeSettings::Volume *> *VolumeSettings::GetVolumes() { return &volumes; }
...
void VolumeSettings::SetActiveVolume(int index) {
  if (index < 0 || index >= volumes.count()) {
    throw new std::out_of_range("invalid index given to VolumeSettings::SetActiveVolume");
  }
  ...
  if (volumes[index]->IsAutoSectorCount()) {
    ...
    sbVolSize->setEnabled(false);
  } else {
    cbVolSizeAuto->setCheckState(Qt::CheckState::Unchecked);
    sbVolSize->setEnabled(true);
  }
  ...
  // Update the auto inode count check box
  if (volumes[index]->IsAutoInodeCount()) {
    cbInodeCountAuto->setCheckState(Qt::CheckState::Checked);
    sbInodeCount->setEnabled(false);
  } else {
    cbInodeCountAuto->setCheckState(Qt::CheckState::Unchecked);
    sbInodeCount->setEnabled(true);
  }
  ...
  if (volumes[index]->IsAutoSectorSize()) {
    ...
    cmbSectorSize->setEnabled(false);
  } else {
    cbSectorSizeAuto->setCheckState(Qt::CheckState::Unchecked);
    cmbSectorSize->setEnabled(true);
  }

  // Update the sector size combo box
  cmbSectorSize->setCurrentText(l.toString(static_cast<qlonglong>(volumes[index]->GetStSectorSize()->GetValue())));
...
void VolumeSettings::AddVolume() {
  ...
  volumes.append(new Volume(name, wbtnPathPrefix, wbtnSectorSize, wbtnVolSize, wbtnVolOff, wbtnInodeCount,
                            wbtnAtomicWrite, wbtnDiscardSupport, wbtnIoRetries));
  volTick++;
...
bool VolumeSettings::GetDiscardsSupported() {
  for (int i = 0; i < volumes.count(); i++) {
    if (QString::compare(volumes[i]->GetStDiscardSupport()->GetValue(), gpszSupported, Qt::CaseInsensitive) == 0) {
...
{\n");
  ...
  for (int i = 0; i < volumes.count(); i++) {
    ...
    if (volumes[i]->IsAutoInodeCount()) {
      inodeCount = QString("INODE_COUNT_AUTO");
    } else {
      inodeCount = QString::number(volumes[i]->GetStInodeCount()->GetValue()) + QString("U");
    }

    toReturn += QString("    { ") + sectorSize + QString(", ") + sectorCount + QString(", ") +
                QString::number(volumes[i]->GetStSectorOff()->GetValue()) + QString("U, ") +
                (QString::compare(volumes[i]->GetStAtomicWrite()->GetValue(), gpszSupported, Qt::CaseInsensitive) == 0
                     ? QString("true")
                     : QString("false")) +
                QString(", ") + inodeCount + QString(", ") +
                QString::number(volumes[i]->GetStBlockIoRetries()->GetValue()) + QString("U");
    ...
    if (allSettings.cbsAutomaticDiscards->GetValue() || allSettings.cbsPosixFstrim->GetValue()) {
      toReturn +=
          (QString::compare(volumes[i]->GetStDiscardSupport()->GetValue(), gpszSupported, Qt::CaseInsensitive) == 0
               ? QString(", true")
               : QString(", false"));
    }
    ...
    if (allSettings.rbtnsUsePosix->GetValue()) {
      ...
      toReturn += QString(", \"") + volName + QString("\"");
    }
...
void VolumeSettings::ParseCodefile(const QString &text, QStringList &notFound, QStringList &notParsed) {
  ...
  while (!failure) {
    ...
    if (!rem.hasMatch() || rem.lastCapturedIndex() < 2) {
      ...
      // name if not found.
      pathPrefix = QString("VOL") + QString::number(currVolIndex) + QString(":");
    } else {
      pathPrefix = rem.captured(2);
    }

    Volume *newVol = new Volume(pathPrefix, wbtnPathPrefix, wbtnSectorSize, wbtnVolSize, wbtnVolOff, wbtnInodeCount,
                                wbtnAtomicWrite, wbtnDiscardSupport, wbtnIoRetries);

    if ((QString::compare(strValues[0], "SECTOR_SIZE_AUTO") == 0) || (QString::compare(strValues[0], "0U") == 0) ||
        (QString::compare(strValues[0], "0") == 0)) {
      newVol->SetAutoSectorSize(true);
    } else {
      newVol->SetAutoSectorSize(false);
      parseAndSet(newVol->GetStSectorSize(), strValues[0], notParsed, pathPrefix + QString(" sector size"));
    }
    if ((QString::compare(strValues[1], "SECTOR_COUNT_AUTO") == 0) || (QString::compare(strValues[1], "0U") == 0) ||
        (QString::compare(strValues[1], "0") == 0)) {
      newVol->SetAutoSectorCount(true);
    } else {
      newVol->SetAutoSectorCount(false);
      parseAndSet(newVol->GetStSectorCount(), strValues[1], notParsed, pathPrefix + QString(" sector count"));
    }
    parseAndSet(newVol->GetStSectorOff(), strValues[2], notParsed, pathPrefix + QString(" sector offset"));
    ...
    if ((QString::compare(strValues[4], "INODE_COUNT_AUTO") == 0) || (QString::compare(strValues[4], "0U") == 0) ||
        (QString::compare(strValues[4], "0") == 0)) {
      newVol->SetAutoInodeCount(true);
    } else {
      newVol->SetAutoInodeCount(false);
      parseAndSet(newVol->GetStInodeCount(), strValues[4], notParsed, pathPrefix + QString(" inode count"));
    }

    // Don't complain if I/O retries setting isn't found (see
    // comments above).
    if (strValues.count() > 5) {
      parseAndSet(newVol->GetStBlockIoRetries(), strValues[5], notParsed, pathPrefix + QString(" block I/O retries"));
    }
...
void VolumeSettings::deselectVolume(int index) {
  ...
  allSettings.cmisBlockSize->notifyList.removeOne(volumes[index]->GetStSectorSize());
  allSettings.cmisBlockSize->notifyList.removeOne(volumes[index]->GetStSectorCount());
  allSettings.cmisBlockSize->notifyList.removeOne(volumes[index]->GetStSectorOff());
  allSettings.cmisBlockSize->notifyList.removeOne(volumes[index]->GetStInodeCount());
  allSettings.rbtnsUsePosix->notifyList.removeOne(volumes[index]->GetStInodeCount());
  allSettings.rbtnsUsePosix->notifyList.removeOne(volumes[index]->GetStName());
}

// Helper function for ParseCodeFile
template <typename T>
void VolumeSettings::parseAndSet(Setting<T> *setting, const QString &strValue, QStringList &notParsed,
...
void VolumeSettings::checkSetVolumeCount() {
  ...
  QString msg;
  Validity v = stVolumeCount.CheckValid((unsigned long)volumes.count(), msg);
...
void VolumeSettings::updateVolSizeBytes() {
  ...
  if (volumes[activeIndex]->IsAutoSectorCount() || volumes[activeIndex]->IsAutoSectorSize()) {
    ...
    labelVolSizeBytes->setText(
        FormatSize(static_cast<qulonglong>(volumes[activeIndex]->GetStSectorSize()->GetValue()) *
...
void VolumeSettings::updateVolOffBytes() {
  labelVolOffBytes->setText(FormatSize(static_cast<qulonglong>(volumes[activeIndex]->GetStSectorSize()->GetValue()) *
...
void VolumeSettings::lePathPrefix_textChanged(const QString &text) {
  ...
  if (usePosix) {
    ...
    listVolumes->item(activeIndex)->setText(QString("Volume ") + QString::number(activeIndex));
  }
}
...
void VolumeSettings::cmbSectorSize_currentIndexChanged(int index) {
  ...
  volumes[activeIndex]->GetStSectorSize()->ProcessInput(cmbSectorSize->itemText(index));
...
void VolumeSettings::cbInodeCountAuto_stateChanged(int state) {
  volumes[activeIndex]->SetAutoInodeCount(state == Qt::Checked);
  sbInodeCount->setEnabled(state == Qt::Unchecked);
}
...
void VolumeSettings::cmbAtomicWrite_currentIndexChanged(int index) {
  ...
  volumes[activeIndex]->GetStAtomicWrite()->ProcessInput(cmbAtomicWrite->itemText(index));
}
...
void VolumeSettings::cmbDiscardSupport_currentIndexChanged(int index) {
  ...
  volumes[activeIndex]->GetStDiscardSupport()->ProcessInput(cmbDiscardSupport->itemText(index));
}
...
void VolumeSettings::cbEnableRetries_stateChanged(int state) {
  ...
  if (state == Qt::Checked) {
    volumes[activeIndex]->GetStBlockIoRetries()->ProcessInput(sbNumRetries->text());
  } else {
    // Unchecked -> disable retries -> max 0 retries.
    volumes[activeIndex]->GetStBlockIoRetries()->ProcessInput("0");
  }
}
...
void VolumeSettings::btnRemSelected_clicked() { RemoveActiveVolume(); }
