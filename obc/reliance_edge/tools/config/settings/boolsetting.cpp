...
BoolSetting::BoolSetting(QString macroName, bool defaultValue, std::function<Validity(bool, QString &)> validator,
                         WarningBtn *btnWarn)
    : Setting<bool>(macroName, defaultValue, validator), btnWarning(btnWarn) {}
...
bool BoolSetting::TryParse(const QString &toParse, bool &out) {
  ...
  // sense reasons.
  if (QString::compare(toParse, "1") == 0 || QString::compare(toParse, "true", Qt::CaseInsensitive) == 0) {
    out = true;
    return true;
  } else if (QString::compare(toParse, "0") == 0 || QString::compare(toParse, "false", Qt::CaseInsensitive) == 0) {
    out = false;
    return true;
  } else
    return false;
