...
class Setting : public SettingBase {
 public:
  ...
  ///                     this setting
  ///
  Setting(QString macroName, T defaultValue, std::function<Validity(T, QString &)> validator);
  ...
  ///
  virtual T GetValue(void) { return value; }

  ///
  /// \brief  Returns the macro name associated with this setting
  ///
  QString GetMacroName() { return name; }
...
template <typename T>
Setting<T>::Setting(QString macroName, T defaultValue, std::function<Validity(T, QString &)> validator)
    : name(macroName), defValue(defaultValue), value(defaultValue), validateFn(validator) {
  if (!validateFn) {
