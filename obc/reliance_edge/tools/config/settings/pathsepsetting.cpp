...
PathSepSetting::PathSepSetting(QString macroName, QString defaultValue,
                               std::function<Validity(QString, QString &)> validator, QComboBox *cmb, QLineEdit *le,
                               WarningBtn *btnWarn)
    : StrSetting(macroName, defaultValue, validator, btnWarn), comboBox(cmb), lineEdit(le) {
  if (!cmb || !le) {
    throw new std::invalid_argument("cmb and le cannot be null");
  }

  setUi();
  connect(cmb, SIGNAL(currentIndexChanged(int)), this, SLOT(comboBox_currentIndexChanged(int)));
  connect(le, SIGNAL(textChanged(QString)), this, SLOT(lineEdit_textChanged(QString)));
}
...
void PathSepSetting::setUi() {
  if (QString::compare(value, "/") == 0) {
    ...
    lineEdit->setVisible(false);
  } else if (QString::compare(value, "\\") == 0 || QString::compare(value, "\\\\") == 0)  // double-escaped backslash
  {
    comboBox->setCurrentText("\\");
    lineEdit->setVisible(false);
  } else {
...
void PathSepSetting::lineEdit_textChanged(const QString &text) { ProcessInput(text); }
