...
void Input::TryLoad() {
  ...
  if (fileDialog == NULL) {
    fileDialog = new FileDialog(parentWindow, QFileDialog::AcceptOpen, QFileDialog::ExistingFile);
  }
...
bool Input::getFile(const QString &filePath, QString &fileContent) {
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    if (stmIn.status() != QTextStream::Ok || fileContent.isNull() || fileContent.isEmpty()) {
      success = false;
    }
  }
