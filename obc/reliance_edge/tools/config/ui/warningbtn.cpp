...
WarningBtn::WarningBtn(QWidget *parent) : QWidget(parent), ui(new Ui::WarningBtn) {
  ui->setupUi(this);
  ...
  connect(ui->toolButton, SIGNAL(clicked()), this, SIGNAL(clicked()));

  ui->toolButton->setVisible(false);
}

WarningBtn::~WarningBtn() { delete ui; }
...
bool WarningBtn::event(QEvent *e) {
  if (e->type() == QEvent::Enter) {
    if (!currMsg.isNull() && !currMsg.isEmpty()) {
      ...
      QToolTip::showText(toolTipPoint, currMsg, ui->toolButton);
    }
  }
