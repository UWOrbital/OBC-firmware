...
Validity validatePathSepChar(QString value, QString &msg) {
  ...
  if (value.startsWith('\\')) {
    if (value.length() == 2 && (value[1] == '\\' || value[1] == 'a' || value[1] == 'b' || value[1] == 'f' ||
                                value[1] == 'n' || value[1] == 'r' || value[1] == 't' || value[1] == 'v' ||
...
Validity validateVolName(QString value, QString &msg) {
  ...
  QList<VolumeSettings::Volume *> *const volumes = volumeSettings->GetVolumes();

  // Check for duplicates to this name
  for (int i = 0; i < volumes->count(); i++) {
    if (i == ignoreIndex) continue;
    if ((*volumes)[i]->GetStName()->GetValue() == value) {
      msg = QString("Volume name must be unique. Duplicate volume name ") + value + QString(".");
      return Invalid;
    }
  }
...
Validity validateVolSectorSize(unsigned long value, QString &msg) {
  ...
  Q_ASSERT(allSettings.cmisBlockSize != NULL);
  if (!isPowerOfTwo(value) || value < 128 || value > 65536)  // 2^16. Same max in block size
  {
  ...
  if (!isAutoCount && !isAutoSize) {
    ...
    if ((inodeTableStartBN + value * 2) > blockCount) {
      unsigned long currMax =
          ((long)blockCount - (long)inodeTableStartBN > 0 ? (blockCount - inodeTableStartBN) / 2 : 0);

      msg = QString("Inode count too high; limited by sector count. Current max: ") + QString::number(currMax) +
            QString(".");
      return Invalid;
    }
  }
...
Validity validateByteOrder(QString value, QString &msg) {
  if (value.startsWith("big", Qt::CaseInsensitive) || value.startsWith("little", Qt::CaseInsensitive)) {
    return Valid;
  } else {
    Q_ASSERT(false);  // The associated QComboBox should not allow this.
...
Validity validateCrc(QString value, QString &msg) {
  if (QString::compare(value, crcBitwise) == 0 || QString::compare(value, crcSarwate) == 0 ||
      QString::compare(value, crcSlice) == 0) {
    // Actual values
    return Valid;
  } else if (value.startsWith("bitwise", Qt::CaseInsensitive) || value.startsWith("sarwate", Qt::CaseInsensitive) ||
...
Validity validateDirectPointers(unsigned long value, QString &msg) {
  ...
  if (value > inodeEntries - allSettings.sbsIndirectPtrs->GetValue()) {
    msg = QString("Too many direct pointers. Current maximum based on other settings: ") +
          QString::number(inodeEntries - allSettings.sbsIndirectPtrs->GetValue()) + QString(".");
    return Invalid;
  }
...
Validity validateIndirectPointers(unsigned long value, QString &msg) {
  ...
  if (value > inodeEntries - allSettings.sbsDirectPtrs->GetValue()) {
    msg = QString("Too many indirect pointers. Current maximum based on other settings: ") +
          QString::number(inodeEntries - allSettings.sbsDirectPtrs->GetValue()) + QString(".");
    return Invalid;
  }
...
Validity validateAllocatedBuffers(unsigned long value, QString &msg) {
  ...
  if (value > maximumBufferCount) {
    msg = QString("Too many allocated buffers. Current maximum based on block size: ") +
          QString::number(maximumBufferCount) + QString(".");
    return Invalid;
  } else if (value > 255) {
  ...
  ulong dindirPointers =
      (getInodeEntries() - allSettings.sbsDirectPtrs->GetValue()) - allSettings.sbsIndirectPtrs->GetValue();
  ...
  if (allSettings.cbsReadonly->GetValue() || !allSettings.rbtnsUsePosix->GetValue()) {
  ...
  if (value < minimumBufferCount) {
    if (allSettings.cbsPosixRename->GetValue()) {
      ...
      msg = QString("Too few allocated buffers. Current minimum based on other settings: ") +
            QString::number(minimumBufferCount) + QString(".");
    }
    return Invalid;
  }
...
Validity validateBufferAlignment(unsigned long value, QString &msg) {
  ...
  if (value > blockSize) {
    msg = QString("Alignment should not exceed block size. Current maximum based on block size: ") +
          QString::number(blockSize) + QString(".");
    return Invalid;
  }
...
Validity validateBufferWriteGather(unsigned long value, QString &msg) {
  ...
  if (value > maximum4GB) {
    msg = QString("Write-gather size exceeds the 4GB maximum of ") + QString::number(maximum4GB) + QString(".");
    return Invalid;
  }

  if (value > maximumBufferLimit) {
    msg = QString("Write-gather size exceeds the total buffers maximum of ") + QString::number(maximumBufferLimit) +
          QString(".");
    return Invalid;
  }
  ...
  if (value) {
    ...
    if (bigValue < minimumBlockSizeLimit) {
      msg = QString("Write-gather size doesn't meet the block size minimum of ") +
            QString::number(minimumBlockSizeLimit) + QString(".");
      return Invalid;
    }
...
Validity validateTransactVolFull(bool value, QString &msg) {
  ...
  Q_ASSERT(allSettings.cbsReadonly != NULL);
  if (!value && !allSettings.cbsTrManual->GetValue() && !allSettings.cbsReadonly->GetValue()) {
    msg = "Automatic transaction on volume full recommended except in special cases.";
    return Warning;
  }
  return Valid;
...
unsigned long getInodeEntries() {
  ...
  unsigned long inodeHeaderSize = 16 + 8 + (allSettings.cbsInodeBlockCount->GetValue() ? 4 : 0) +
                                  (allSettings.cbsInodeTimestamps->GetValue() ? 12 : 0) +
                                  ((posix && allSettings.cbsPosixOwnerPerm->GetValue()) ? 8 : 0) + 4 + (posix ? 4 : 0) +
...
static bool isPowerOfTwo(unsigned long value) { return (value != 0) && ((value & (value - 1)) == 0); }
