...
REDSTATUS RedMetadataIterate(const MDITERPARAM *pParam) {
  ...
  if ((pParam == NULL) || (pParam->bVolNum >= REDCONF_VOLUME_COUNT) || (pParam->pfnCallback == NULL)) {
    ret = -RED_EINVAL;
    goto Out;
  }
    ...
    */
    ...
    if (hdr.ulCRC != RedCrcNode(pMR)) {
      fprintf(stderr, "Invalid metaroot CRC in block %lu: found 0x%08lx, expected 0x%08lx\n", (unsigned long)ulBlock,
              (unsigned long)hdr.ulCRC, (unsigned long)RedCrcNode(pMR));
      fValid = false;
    }
    ...
    if (pCtx->pParam->fVerify) {
      ...
      if (hdr.ulCRC != RedCrcNode(pImap)) {
        fprintf(stderr, "Invalid imap CRC in block %lu: found 0x%08lx, expected 0x%08lx\n", (unsigned long)ulBlock,
                (unsigned long)hdr.ulCRC, (unsigned long)RedCrcNode(pImap));
        ret = -RED_EIO;
      }
    ...
    if (pCtx->pParam->fVerify) {
      ...
      if (hdr.ulCRC != RedCrcNode(pInode)) {
        fprintf(stderr, "Invalid inode CRC in block %lu: found 0x%08lx, expected 0x%08lx\n", (unsigned long)ulBlock,
                (unsigned long)hdr.ulCRC, (unsigned long)RedCrcNode(pInode));
        ret = -RED_EIO;
      }
  ...
  if (pCtx->pParam->fVerify) {
    ...
    if (ulOwnerInode != ulInode) {
      fprintf(stderr, "Invalid double indirect inode in block %lu: found %lu, expected %lu\n", (unsigned long)ulBlock,
              (unsigned long)ulOwnerInode, (unsigned long)ulInode);
      ret = -RED_EIO;
    }
  ...
  if (pCtx->pParam->fVerify) {
    ...
    if (hdr.ulCRC != RedCrcNode(pIndir)) {
      fprintf(stderr, "Invalid indirect CRC in block %lu: found 0x%08lx, expected 0x%08lx\n", (unsigned long)ulBlock,
              (unsigned long)hdr.ulCRC, (unsigned long)RedCrcNode(pIndir));
      ret = -RED_EIO;
    }
  ...
  if (pCtx->pParam->fVerify && (pCtx->ulVersion >= RED_DISK_LAYOUT_DIRCRC)) {
    ...
    if (hdr.ulCRC != RedCrcNode(pDirData)) {
      fprintf(stderr, "Invalid directory CRC in block %lu: found 0x%08lx, expected 0x%08lx\n", (unsigned long)ulBlock,
              (unsigned long)hdr.ulCRC, (unsigned long)RedCrcNode(pDirData));
      ret = -RED_EIO;
    }
...
static NODEHEADER NodeHdrExtract(const void *pBuffer) {
  ...
  */
  if ((hdr.ulSignature == SWAP32(META_SIG_MASTER)) || (hdr.ulSignature == SWAP32(META_SIG_METAROOT)) ||
      (hdr.ulSignature == SWAP32(META_SIG_IMAP)) || (hdr.ulSignature == SWAP32(META_SIG_INODE)) ||
      (hdr.ulSignature == SWAP32(META_SIG_DINDIR)) || (hdr.ulSignature == SWAP32(META_SIG_INDIR)) ||
