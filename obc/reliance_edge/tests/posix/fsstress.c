...
*/
static char *fsstress_getcwd(char *pszBuf, size_t nSize) {
  char *pszRet = NULL;
...
static flist_t flist[FT_nft] = {
    {0, 0, 'd', NULL}, {0, 0, 'f', NULL}, {0, 0, 'l', NULL}, {0, 0, 'c', NULL}, {0, 0, 'r', NULL},
};
...
static int delete_tree(const char *path) {
  ...
  if (!S_ISDIR(sb.st_mode)) return unlink(path) ? errno : 0;
  ...
  while ((dep = readdir(dp)) != NULL) {
    ...
    if (childpath[strlen(childpath) - 1] != '/') strcat(childpath, "/");
...
static void check_cwd(void) {
  ...
  if (stat64(".", &statbuf) == 0 && statbuf.st_ino == top_ino) return;
  chdir(homedir);
...
static int creat_path(pathname_t *name, mode_t mode) {
  ...
  if (rval >= 0 || errno != RED_ENAMETOOLONG) return rval;
...
static void dcache_enter(int dirid, int slot) { dcache[dirid % NDCACHE] = slot; }
...
static fent_t *dcache_lookup(int dirid) {
  ...
  if (i >= 0 && (fep = &flist[FT_DIR].fents[i])->id == dirid) return fep;
  return NULL;
}
...
static void dcache_purge(int dirid) {
  ...
  if (*dcp >= 0 && flist[FT_DIR].fents[*dcp].id == dirid) *dcp = -1;
}
...
static void del_from_flist(int ft, int slot) {
  ...
  ftp = &flist[ft];
  if (ft == FT_DIR) dcache_purge(ftp->fents[slot].id);
  if (slot != ftp->nfiles - 1) {
    if (ft == FT_DIR) dcache_purge(ftp->fents[ftp->nfiles - 1].id);
...
static fent_t *dirid_to_fent(int dirid) {
  ...
  if ((fep = dcache_lookup(dirid))) return fep;
  flp = &flist[FT_DIR];
...
static void fix_parent(int oldid, int newid) {
  ...
  for (i = 0, flp = flist; i < FT_nft; i++, flp++) {
    for (j = 0, fep = flp->fents; j < flp->nfiles; j++, fep++) {
      if (fep->parent == oldid) fep->parent = newid;
    }
  }
}
...
static int get_fname(int which, long r, pathname_t *name, flist_t **flpp, fent_t **fepp, int *v) {
  ...
  for (i = 0, c = 0, flp = flist; i < FT_nft; i++, flp++) {
    if (which & (1 << i)) {
      if (x < c + flp->nfiles) {
        fep = &flp->fents[x - c];
        if (name) fent_to_name(name, flp, fep);
        if (flpp) *flpp = flp;
        if (fepp) *fepp = fep;
        *v = verbose;
        for (j = 0; !*v && j < ilistlen; j++) {
  ...
  */
  if (!bucket_count) {
    ...
    for (i = 0; i < bucket_count; i++) {
      if (bucket_value > 128 || bucket_value >= (int)MAXNAMELEN - 1) break;
...
static DIR *opendir_path(pathname_t *name) {
  ...
  if (rval || errno != RED_ENAMETOOLONG) return rval;
  separate_pathname(name, buf, &newname);
  if (chdir(buf) == 0) {
...
static void chown_f(int opno, long r) {
  ...
  init_pathname(&f);
  if (!get_fname(FT_ANYm, r, &f, NULL, NULL, &v)) append_pathname(&f, ".");
  u = (uid_t)random();
  nbits = (int)(random() % 32);
  u &= (1 << nbits) - 1;
  e = lchown_path(&f, u, -1) < 0 ? errno : 0;
  check_cwd();
  if (v) printf("%d/%d: chown %s %d %d\n", procid, opno, f.path, u, e);
  free_pathname(&f);
}
#endif
...
static void creat_f(int opno, long r) {
  ...
  if (v) RedPrintf("%d/%d: creat %s x:%d %d %d\n", procid, opno, f.path, esz, e, e1);
  free_pathname(&f);
}
...
static void fdatasync_f(int opno, long r) {
  ...
  if (!get_fname(FT_REGFILE, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: fdatasync - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  ...
  check_cwd();
  if (fd < 0) {
    if (v) RedPrintf("%d/%d: fdatasync - open %s failed %d\n", procid, opno, f.path, e);
    free_pathname(&f);
    return;
  }
  e = fdatasync(fd) < 0 ? errno : 0;
  if (v) RedPrintf("%d/%d: fdatasync %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
  close(fd);
}
...
static void fsync_f(int opno, long r) {
  ...
  init_pathname(&f);
  if (!get_fname(FT_REGFILE, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: fsync - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  fd = open_path(&f, O_WRONLY);
  ...
  check_cwd();
  if (fd < 0) {
    if (v) RedPrintf("%d/%d: fsync - open %s failed %d\n", procid, opno, f.path, e);
    free_pathname(&f);
    return;
  }
  e = fsync(fd) < 0 ? errno : 0;
  if (v) RedPrintf("%d/%d: fsync %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
  close(fd);
}
...
static void getdents_f(int opno, long r) {
  ...
  init_pathname(&f);
  if (!get_fname(FT_DIRm, r, &f, NULL, NULL, &v)) append_pathname(&f, ".");
  dir = opendir_path(&f);
  check_cwd();
  if (dir == NULL) {
    if (v) RedPrintf("%d/%d: getdents - can't open %s\n", procid, opno, f.path);
    free_pathname(&f);
    return;
  }
  while (readdir64(dir) != NULL) continue;
  if (v) RedPrintf("%d/%d: getdents %s 0\n", procid, opno, f.path);
  free_pathname(&f);
  closedir(dir);
}
...
static void link_f(int opno, long r) {
  ...
  if (!get_fname(FT_NOTDIR, r, &f, &flp, NULL, &v1)) {
    if (v1) RedPrintf("%d/%d: link - no file\n", procid, opno);
    free_pathname(&f);
    return;
  }
  ...
  check_cwd();
  if (e == 0) add_to_flist((int)(flp - flist), id, parid);
  if (v) RedPrintf("%d/%d: link %s %s %d\n", procid, opno, f.path, l.path, e);
  free_pathname(&l);
  free_pathname(&f);
}
...
static void mkdir_f(int opno, long r) {
  ...
  check_cwd();
  if (e == 0) add_to_flist(FT_DIR, id, parid);
  if (v) RedPrintf("%d/%d: mkdir %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
}
...
static void read_f(int opno, long r) {
  ...
  init_pathname(&f);
  if (!get_fname(FT_REGFILE, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: read - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  ...
  check_cwd();
  if (fd < 0) {
    if (v) RedPrintf("%d/%d: read - open %s failed %d\n", procid, opno, f.path, e);
    free_pathname(&f);
    return;
  }
  if (fstat64(fd, &stb) < 0) {
    if (v) RedPrintf("%d/%d: read - fstat64 %s failed %d\n", procid, opno, f.path, errno);
    free_pathname(&f);
    close(fd);
    return;
  }
  if (stb.st_size == 0) {
    if (v) RedPrintf("%d/%d: read - %s zero size\n", procid, opno, f.path);
    free_pathname(&f);
    close(fd);
    return;
  }
...
static void rename_f(int opno, long r) {
  ...
  if (!get_fname(FT_ANYm, r, &f, &flp, &fep, &v1)) {
    if (v1) RedPrintf("%d/%d: rename - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  ...
  if (v) RedPrintf("%d/%d: rename %s to %s %d\n", procid, opno, f.path, newf.path, e);
  free_pathname(&newf);
  free_pathname(&f);
}
...
static void rmdir_f(int opno, long r) {
  ...
  if (!get_fname(FT_DIRm, r, &f, NULL, &fep, &v)) {
    if (v) RedPrintf("%d/%d: rmdir - no directory\n", procid, opno);
    free_pathname(&f);
    return;
  }
  e = rmdir_path(&f) < 0 ? errno : 0;
  check_cwd();
  if (e == 0) del_from_flist(FT_DIR, (int)(fep - flist[FT_DIR].fents));
  if (v) RedPrintf("%d/%d: rmdir %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
}
...
static void stat_f(int opno, long r) {
  ...
  if (!get_fname(FT_ANYm, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: stat - no entries\n", procid, opno);
    free_pathname(&f);
    return;
  }
  e = lstat64_path(&f, &stb) < 0 ? errno : 0;
  check_cwd();
  if (v) RedPrintf("%d/%d: stat %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
}
...
static void truncate_f(int opno, long r) {
  ...
  init_pathname(&f);
  if (!get_fname(FT_REGFILE, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: truncate - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  ...
  check_cwd();
  if (e > 0) {
    if (v) RedPrintf("%d/%d: truncate - stat64 %s failed %d\n", procid, opno, f.path, e);
    free_pathname(&f);
    return;
  }
  lr = ((__int64_t)random() << 32) + random();
  ...
  check_cwd();
  if (v) RedPrintf("%d/%d: truncate %s %lld %d\n", procid, opno, f.path, (long long)off, e);
  free_pathname(&f);
}
...
static void unlink_f(int opno, long r) {
  ...
  if (!get_fname(FT_NOTDIR, r, &f, &flp, &fep, &v)) {
    if (v) RedPrintf("%d/%d: unlink - no file\n", procid, opno);
    free_pathname(&f);
    return;
  }
  e = unlink_path(&f) < 0 ? errno : 0;
  check_cwd();
  if (e == 0) del_from_flist((int)(flp - flist), (int)(fep - flp->fents));
  if (v) RedPrintf("%d/%d: unlink %s %d\n", procid, opno, f.path, e);
  free_pathname(&f);
}
...
static void write_f(int opno, long r) {
  ...
  if (!get_fname(FT_REGm, r, &f, NULL, NULL, &v)) {
    if (v) RedPrintf("%d/%d: write - no filename\n", procid, opno);
    free_pathname(&f);
    return;
  }
  fd = open_path(&f, O_WRONLY);
  ...
  check_cwd();
  if (fd < 0) {
    if (v) RedPrintf("%d/%d: write - open %s failed %d\n", procid, opno, f.path, e);
    free_pathname(&f);
    return;
  }
  if (fstat64(fd, &stb) < 0) {
    if (v) RedPrintf("%d/%d: write - fstat64 %s failed %d\n", procid, opno, f.path, errno);
    free_pathname(&f);
    close(fd);
    return;
  }
  ...
  free(buf);
  if (v) RedPrintf("%d/%d: write %s [%lld,%ld] %d\n", procid, opno, f.path, (long long)off, (long int)len, e);
  free_pathname(&f);
  close(fd);
}
