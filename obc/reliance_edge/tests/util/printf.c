...
static uint32_t ProcessFormatSegment(char *pcBuffer, uint32_t ulBufferLen, const char *pszFormat, PRINTFORMAT *pFormat,
...
static uint32_t ULLtoA(char *pcBuffer, uint32_t ulBufferLen, uint64_t ullNum, bool fHex, uint32_t ulFillLen,
                       char cFill);
static uint32_t FinishToA(const char *pcDigits, uint32_t ulDigits, char *pcOutBuffer, uint32_t ulBufferLen,
                          uint32_t ulFillLen, char cFill);
...
int32_t RedVSNPrintf(char *pcBuffer, uint32_t ulBufferLen, const char *pszFormat, va_list arglist) {
  ...
  while ((pszFormat[ulFmtIdx] != '\0') && (ulBufIdx < ulBufferLen)) {
    ...
    */
    ulWidth =
        ProcessFormatSegment(&pcBuffer[ulBufIdx], ulBufferLen - ulBufIdx, &pszFormat[ulFmtIdx], &fmt, &ulSpecifierLen);
    if (ulWidth) {
    ...
    switch (fmt.type) {
      ...
      case PRFMT_POINTER: {
        ...
        if (sizeof(void *) > 4U) {
          ...
          ulBufIdx +=
              ULLtoA(&pcBuffer[ulBufIdx], ulBufferLen - ulBufIdx, ullPtrVal, true, fmt.ulFillLen, fmt.cFillChar);
        } else {
          /*  Attempt to quiet warnings.
           */
          uintptr_t ptrval = (uintptr_t)ptr;
