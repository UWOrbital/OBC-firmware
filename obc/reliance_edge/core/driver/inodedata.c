  ...
  if (!CINODE_IS_MOUNTED(pInode) || (ulBlock >= INODE_DATA_BLOCKS)) {
...
#if DINDIR_POINTERS > 0U
    {
      ...
      uint16_t uInodeEntry =
          (uint16_t)((ulDindirRangeOffset / DINDIR_DATA_BLOCKS) + REDCONF_DIRECT_POINTERS + REDCONF_INDIRECT_POINTERS);
...
static REDSTATUS ReadUnaligned(CINODE *pInode, uint64_t ullStart, uint32_t ulLen, uint8_t *pbBuffer) {
   ...
   */
  if (((ullStart >> BLOCK_SIZE_P2) != (((ullStart + ulLen) - 1U) >> BLOCK_SIZE_P2)) || (pbBuffer == NULL)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
