...
REDSTATUS RedInodeCreate(CINODE *pInode, CINODE *pPInode, uint16_t uMode) {
  ...
  /*  pPInode must be a valid, mounted cached inode, unless we are creating
      ...
      parent inode.
  */
  if ((pInode == NULL) || (!CINODE_IS_MOUNTED(pPInode) && ((pPInode != NULL) || (pInode->ulInode != INODE_ROOTDIR))) ||
      ((uMode & ~RED_S_IFVALID) != 0U) ||
      (((uMode & RED_S_IFMT) != RED_S_IFREG) && ((uMode & RED_S_IFMT) != RED_S_IFDIR)
...
#endif
  {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        ret =
            RedBufferGet(InodeBlock(pInode->ulInode, bWriteableWhich),
