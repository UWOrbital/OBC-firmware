...
REDSTATUS RedBufferGet(uint32_t ulBlock, uint16_t uFlags, void **ppBuffer) {
  ...
  if ((ulBlock >= gpRedVolume->ulBlockCount) || ((uFlags & BFLAG_MASK) != uFlags) ||
      (((uFlags & BFLAG_NEW) != 0U) && ((uFlags & BFLAG_DIRTY) == 0U)) || !BFLAG_TYPE_IS_VALID(uFlags) ||
      (ppBuffer == NULL)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
...
REDSTATUS RedBufferFlushRange(uint32_t ulBlockStart, uint32_t ulBlockCount) {
  ...
  if ((ulBlockStart >= gpRedVolume->ulBlockCount) || ((gpRedVolume->ulBlockCount - ulBlockStart) < ulBlockCount) ||
      (ulBlockCount == 0U)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
    ...
    for (bIdx = 0U; bIdx < REDCONF_BUFFER_COUNT; bIdx++) {
      ...
      if ((pHead->bVolNum == gbRedVolNum) && (pHead->ulBlock != BBLK_INVALID) &&
          ((pHead->uFlags & BFLAG_DIRTY) != 0U) && (pHead->ulBlock >= ulBlockStart) &&
...
void RedBufferBranch(const void *pBuffer, uint32_t ulBlockNew) {
  ...
  if (!BufferToIdx(pBuffer, &bIdx) || (ulBlockNew >= gpRedVolume->ulBlockCount)) {
    REDERROR();
  } else {
    BUFFERHEAD *pHead = &gBufCtx.aHead[bIdx];
...
REDSTATUS RedBufferDiscardRange(uint32_t ulBlockStart, uint32_t ulBlockCount) {
  ...
  if ((ulBlockStart >= gpRedVolume->ulBlockCount) || ((gpRedVolume->ulBlockCount - ulBlockStart) < ulBlockCount) ||
    ...
    for (bIdx = 0U; bIdx < REDCONF_BUFFER_COUNT; bIdx++) {
      ...
      if ((pHead->bVolNum == gbRedVolNum) && (pHead->ulBlock != BBLK_INVALID) && (pHead->ulBlock >= ulBlockStart) &&
          (pHead->ulBlock < (ulBlockStart + ulBlockCount))) {
        if (pHead->bRefCount == 0U) {
          pHead->ulBlock = BBLK_INVALID;
...
REDSTATUS RedBufferReadRange(uint32_t ulBlockStart, uint32_t ulBlockCount, uint8_t *pbDataBuffer) {
  ...
  if ((ulBlockStart >= gpRedVolume->ulBlockCount) || ((gpRedVolume->ulBlockCount - ulBlockStart) < ulBlockCount) ||
      (ulBlockCount == 0U) || (pbDataBuffer == NULL)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
#if REDCONF_READ_ONLY == 0
...
static bool BufferToIdx(const void *pBuffer, uint8_t *pbIdx) {
  ...
  if (PTR_IS_ARRAY_ELEMENT(pBuffer, gBufCtx.pbBlkBuf, REDCONF_BUFFER_COUNT << BLOCK_SIZE_P2, REDCONF_BLOCK_SIZE) &&
    ...
    */
    if ((gBufCtx.aHead[bIdx].ulBlock != BBLK_INVALID) && (gBufCtx.aHead[bIdx].bVolNum == gbRedVolNum)) {
      *pbIdx = bIdx;
      fRet = true;
    }
  }
    ...
    */
    ...
    if (bMruIdx < (REDCONF_BUFFER_COUNT - 1U)) {
      /*  Move the buffer index to the back of the MRU array, making it
          the LRU buffer.
      */
      RedMemMove(&gBufCtx.abMRU[bMruIdx], &gBufCtx.abMRU[bMruIdx + 1U],
                 REDCONF_BUFFER_COUNT - ((uint32_t)bMruIdx + 1U));
