...
REDSTATUS RedImapBlockGet(uint8_t bMR, uint32_t ulBlock, bool *pfAllocated) {
  ...
  if ((bMR > 1U) || (ulBlock < gpRedCoreVol->ulInodeTableStartBN) || (ulBlock >= gpRedVolume->ulBlockCount) ||
      (pfAllocated == NULL)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
...
REDSTATUS RedImapBlockState(uint32_t ulBlock, ALLOCSTATE *pState) {
  ...
  if ((ulBlock < gpRedCoreVol->ulInodeTableStartBN) || (ulBlock >= gpRedVolume->ulBlockCount) || (pState == NULL)) {
    REDERROR();
    ret = -RED_EINVAL;
  } else {
    bool fBitCurrent;
