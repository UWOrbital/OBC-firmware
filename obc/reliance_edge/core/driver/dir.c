...
static REDSTATUS DirEntryWrite(CINODE *pPInode, uint32_t ulIdx, uint32_t ulInode, const char *pszName,
                               uint32_t ulNameLen);
    ...
    if ((DirEntryIndexToOffset(ulDeleteIdx) + DIRENT_SIZE) == pPInode->pInodeBuf->ullSize) {
      ...
      */
      if (ret == 0) {
        ...
        */
        if ((DIR_BLOCK_UNUSED_SPACE > 0U) && (ullNewSize >= REDCONF_BLOCK_SIZE) &&
...
*/
REDSTATUS RedDirEntryRename(CINODE *pSrcPInode, const char *pszSrcName, CINODE *pSrcInode, CINODE *pDstPInode,
                            const char *pszDstName, CINODE *pDstInode) {
  REDSTATUS ret = 0;

  if (!CINODE_IS_DIRTY(pSrcPInode) || (pszSrcName == NULL) || (pSrcInode == NULL) || !CINODE_IS_DIRTY(pDstPInode) ||
      (pszDstName == NULL) || (pDstInode == NULL)) {
    ret = -RED_EINVAL;
  ...
  if (!INODE_IS_VALID(ulSrcInode) || !CINODE_IS_MOUNTED(pDstPInode)) {
    ...
    */
    ...
    while ((NextParent.ulInode != ulSrcInode) && (NextParent.ulInode != INODE_ROOTDIR) &&
           (NextParent.ulInode != INODE_INVALID) && (ulIteration < gpRedCoreVol->ulInodeCount)) {
...
static REDSTATUS DirEntryWrite(CINODE *pPInode, uint32_t ulIdx, uint32_t ulInode, const char *pszName,
  ...
  if (!CINODE_IS_DIRTY(pPInode) || (ulIdx >= DIRENTS_MAX) || (!INODE_IS_VALID(ulInode) && (ulInode != INODE_INVALID)) ||
      (pszName == NULL) || (ulNameLen > REDCONF_NAME_MAX) || ((ulNameLen == 0U) != (ulInode == INODE_INVALID))) {
    REDERROR();
    ret = -RED_EINVAL;
  } else if (!pPInode->fDirectory) {
    ret = -RED_ENOTDIR;
