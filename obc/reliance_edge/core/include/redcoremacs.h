...
#define DINDIR_POINTERS_MAX \
  ((INODE_DINDIR_BLOCKS_MAX / DINDIR_DATA_BLOCKS) + (((INODE_DINDIR_BLOCKS_MAX % DINDIR_DATA_BLOCKS) == 0U) ? 0U : 1U))
 ...
 */
#define INODE_IS_VALID(INODENUM) \
  (((INODENUM) >= INODE_FIRST_VALID) && ((INODENUM) < (INODE_FIRST_VALID + gpRedCoreVol->ulInodeCount)))
...
#endif
#elif ((REDCONF_API_POSIX == 1) && (REDCONF_API_POSIX_FTRUNCATE == 1)) || \
