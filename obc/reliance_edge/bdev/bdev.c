...
REDSTATUS RedBDevOpen(uint8_t bVolNum, BDEVOPENMODE mode) {
  ...
  if (ret == 0) {
    ...
    if ((pVolConf->ullSectorCount == SECTOR_COUNT_AUTO) || (pVolConf->ulSectorSize == SECTOR_SIZE_AUTO)) {
      ret = RedOsBDevGetGeometry(bVolNum, &info);
      if (ret == 0) {
        if ((pVolConf->ullSectorCount != SECTOR_COUNT_AUTO) && (pVolConf->ullSectorCount != info.ullSectorCount)) {
          REDERROR();
          ret = -RED_EINVAL;
        } else if ((pVolConf->ulSectorSize != SECTOR_COUNT_AUTO) && (pVolConf->ulSectorSize != info.ulSectorSize)) {
          REDERROR();
          ret = -RED_EINVAL;
