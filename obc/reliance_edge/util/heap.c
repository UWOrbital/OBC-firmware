...
*/
static void HeapBlockAlloc(REDHEAPHDR *pBlock, uint32_t ulRequestedSize) {
...
*/
void *RedHeapAlloc(uint32_t ulSize) {
  REDHEAPHDR *pHead;
...
*/
static REDHEAPHDR *HeapBlockFree(REDHEAPHDR *pBlock) {
...
int32_t RedHeapCheck(uint8_t bVerbosity) {
  ...
  while (pHead != NULL) {
    if (bVerbosity >= RED_HEAP_VERBOSITY_HEADERS) {
      REDPRINTF(1U, ("RedHeapCheck() Address=0x%p Next=0x%p Prev=0x%p Sentinel=0x%x Size=0x%lx\n", pHead, pHead->pNext,
                     pHead->pPrev, (unsigned)pHead->ulSentinel, (unsigned long)pHead->ulBlockSize));
    }
  ...
  if (fCorrupt) {
    ...
    REDPRINTF(1U, ("Heap Summary: BlocksAllocated=%3lu BlocksFree=%3lu BytesAllocated=%6lu BytesFree=%6lu\n",
                   (unsigned long)ulAllocated, (unsigned long)ulFree, (unsigned long)ulBytesAllocated,
...
void *RedHeapCalloc(uint32_t ulElements, uint32_t ulElementSize) {
  ...
  if ((ulSize < ulElementSize) || ((ulElements > 0U) && (ulSize > (UINT32_MAX / ulElements)))) {
    pMem = NULL; /* Overflow */
  } else {
    ulSize *= ulElements;
