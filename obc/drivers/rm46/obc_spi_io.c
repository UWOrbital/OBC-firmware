...
static void spiLogErrors(uint32_t spiErr) {
  if (spiErr & SPI_FLAG_DLENERR) LOG_ERROR(OBC_ERR_CODE_SPI_DATA_LENGTH_ERROR);
  if (spiErr & SPI_FLAG_TIMEOUT) LOG_ERROR(OBC_ERR_CODE_SPI_TIMEOUT);
  if (spiErr & SPI_FLAG_PARERR) LOG_ERROR(OBC_ERR_CODE_SPI_PARITY_ERROR);
  if (spiErr & SPI_FLAG_DESYNC) LOG_ERROR(OBC_ERR_CODE_SPI_DESYNC_ERROR);
  if (spiErr & SPI_FLAG_BITERR) LOG_ERROR(OBC_ERR_CODE_SPI_BIT_ERROR);
  if (spiErr & SPI_FLAG_RXOVRNINT) LOG_ERROR(OBC_ERR_CODE_SPI_RX_OVERRUN);
}
