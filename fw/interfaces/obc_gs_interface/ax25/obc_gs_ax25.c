 ...
 */
static obc_gs_error_code_t ax25Unstuff(const packed_ax25_packet_t *packet, unstuffed_ax25_packet_t *unstuffedPacket);
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t sFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket, uint8_t *uplinkData,
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t iFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket, uint8_t *uplinkData,
                                      ax25_addr_t *recvAddress);
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t fcsCalculate(const uint8_t *data, uint16_t *calculatedFcs);
 ...
 */
static obc_gs_error_code_t fcsCheck(const uint8_t *data, uint16_t fcs);
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t bitStuffing(uint8_t *rawData, packed_ax25_packet_t *stuffedData);
...
obc_gs_error_code_t ax25Send(uint8_t *telemData, uint8_t telemDataLen, packed_ax25_packet_t *ax25Data,
  ...
  errCode = fcsCalculate(ax25PacketUnstuffed, &fcs);
  ...
  errCode = bitStuffing(ax25PacketUnstuffed, ax25Data);
  if (errCode != OBC_GS_ERR_CODE_SUCCESS) {
    return errCode;
  }
...
obc_gs_error_code_t ax25Recv(packed_ax25_packet_t *ax25Data, uint8_t *uplinkData, uint8_t uplinkDataLen,
  ...
  unstuffed_ax25_packet_t unstuffedPacket = {0};

  errCode = ax25Unstuff(ax25Data, &unstuffedPacket);
  if (errCode != OBC_GS_ERR_CODE_SUCCESS) {
    return errCode;
  }
  ...
  uint16_t fcs =
      unstuffedPacket
          .data[AX25_INFO_BYTES + AX25_PID_BYTES + AX25_CONTROL_BYTES + AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES]
      << 8;

  fcs |=
      unstuffedPacket
          .data[AX25_INFO_BYTES + AX25_PID_BYTES + AX25_CONTROL_BYTES + AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES + 1];

  errCode = fcsCheck(unstuffedPacket.data, fcs);
  if (errCode != OBC_GS_ERR_CODE_SUCCESS) {
    return errCode;
  }

  if (supervisoryFrameFlag) {
    errCode = sFrameRecv(&unstuffedPacket, uplinkData, recvAddress);
  } else {
    errCode = iFrameRecv(&unstuffedPacket, uplinkData, recvAddress);
  }
...
static obc_gs_error_code_t ax25Unstuff(const packed_ax25_packet_t *packet, unstuffed_ax25_packet_t *unstuffedPacket) {
  if (packet == NULL || unstuffedPacket == NULL) {
    return OBC_GS_ERR_CODE_INVALID_ARG;
  }
...
static obc_gs_error_code_t iFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket, uint8_t *uplinkData,
  ...
  memcpy(uplinkData,
         unstuffedPacket->data + AX25_PID_BYTES + AX25_CONTROL_BYTES + AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES,
         AX25_INFO_BYTES);
  pktReceiveNum++;
  return OBC_GS_ERR_CODE_SUCCESS;
}
