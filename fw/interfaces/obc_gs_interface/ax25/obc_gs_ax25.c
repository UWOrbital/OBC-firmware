 ...
 */
static obc_gs_error_code_t ax25Unstuff(uint8_t *packet, uint16_t packetLen, uint8_t *unstuffedPacket);
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t sFrameRecv(unstuffed_ax25_i_frame_t *unstuffedPacket, uint8_t *uplinkData);
 ...
 * @return obc_gs_error_code_t OBC_GS_ERR_CODE_SUCCESS if it was successful and error code if not
 */
static obc_gs_error_code_t iFrameRecv(unstuffed_ax25_i_frame_t *unstuffedPacket, uint8_t *uplinkData);
 ...
 */
__attribute__((unused))  // TODO: remove this when we actually use this function
static obc_gs_error_code_t
fcsCheck(const uint8_t *data, uint16_t fcs);
...
obc_gs_error_code_t ax25Recv(packed_ax25_i_frame_t *ax25Data, uint8_t *uplinkData, uint8_t uplinkDataLen) {
  obc_gs_error_code_t errCode = OBC_GS_ERR_CODE_SUCCESS;
  ...
  errCode = ax25Unstuff(ax25Data->data, ax25Data->length, unstuffedPacket.data);
  if (errCode != OBC_GS_ERR_CODE_SUCCESS) {
    return errCode;
  }
  ...
  */
  ...
  if (!(unstuffedPacket.data[AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES] & 0x01)) {
    errCode = iFrameRecv(&unstuffedPacket, uplinkData);
  }
  // If the LSB was 1, check if the next bit is a 1 to see if it is a U Frame
    ...
    errCode = sFrameRecv(&unstuffedPacket, uplinkData);
  }
...
static obc_gs_error_code_t sFrameRecv(unstuffed_ax25_i_frame_t *unstuffedPacket, uint8_t *uplinkData) {
  if (unstuffedPacket == NULL || uplinkData == NULL) {
    return OBC_GS_ERR_CODE_INVALID_ARG;
  }
...
static obc_gs_error_code_t fcsCalculate(const uint8_t *data, uint16_t *calculatedFcs) {
  if (data == NULL || calculatedFcs == NULL) {
    return OBC_GS_ERR_CODE_INVALID_ARG;
  }
