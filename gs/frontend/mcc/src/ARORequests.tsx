...
function ARORequests() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/aro-request')
      ...
      .then(data => { setARORequests(data); setLoading(false); });
  }, [])
      ...
      <Table striped bordered hover variant="light">
        ...
        <tbody>
          {loading ? <tr><td colSpan={4}>Loading...</td></tr> :
            aroRequests.map((item) => { return <AROItem key={item.id} id={item.id} longitude={item.longitude} latitude={item.latitude} status={item.status} /> })}
        </tbody>
      </Table>
    </div >
  );
