...
class CSPListener : public itas109::CSerialPortListener {
 public:
  CSPListener(void *pSerialPort, pFunReadEvent pFun) : m_pSerialPort(pSerialPort), m_pFun(pFun){};
...
void CSerialPortAvailablePortInfos(SerialPortInfoArray *portInfoArray) {
  ...
  if (portInfoArray->size > 0) {
    ...
    for (unsigned int i = 0; i < portInfoArray->size; ++i) {
      strncpy(portInfoArray->portInfo[i].portName, portInfos[i].portName, 256);
      strncpy(portInfoArray->portInfo[i].description, portInfos[i].description, 256);
      strncpy(portInfoArray->portInfo[i].hardwareId, portInfos[i].hardwareId, 256);
    }
  } else {
...
void *CSerialPortMalloc() { return new itas109::CSerialPort(); }
...
void CSerialPortInit(void *pSerialPort, const char *portName, int baudRate, Parity parity, DataBits dataBits,
                     StopBits stopbits, FlowControl flowControl, unsigned int readBufferSize) {
  itas109::CSerialPort *pCSP = reinterpret_cast<itas109::CSerialPort *>(pSerialPort);
  if (pCSP) {
    pCSP->init(portName, baudRate, itas109::Parity(parity), itas109::DataBits(dataBits), itas109::StopBits(stopbits),
               itas109::FlowControl(flowControl), readBufferSize);
  }
}
