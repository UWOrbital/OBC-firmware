 ...
 * @return obc_error_code_t		OBC_ERR_CODE_LOG_MSG_SILENCED 	if msgLevel is
 * lower than logging level OBC_ERR_CODE_BUFF_TOO_SMALL		if logged message is
 * too long OBC_ERR_CODE_INVALID_ARG		if file or s are null or if there is an
 * encoding error OBC_ERR_CODE_SUCCESS			if message is successfully logged
