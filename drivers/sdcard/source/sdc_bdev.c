 ...
 *
 * @return sdc_power_t POWER_ON if the SD card is powered on, POWER_OFF
 * otherwise.
 */
...
DSTATUS disk_initialize(uint8_t drv) {
  ...
  for (uint8_t i = 0; i < SDC_ACTION_NUM_ATTEMPTS_DEFAULT; i++) {
    // Reset the card and put it into SPI mode; response should be 0x01 if
    // successful
  ...
  if (sendCMD(SDC_CMD8, DISK_INIT_RESET_ARG) == 1U) {
    ...
    if (ocr[2] == 0x01 && ocr[3] == 0xAA) {
      ...
      for (uint8_t i = 0; i < SDC_ACTION_NUM_ATTEMPTS_DEFAULT; i++) {
        // Begin initialization process with ACMD41 with HCS[bit30] set as
        // argument CMD55 must be sent before ACMD41
