...
/**
 * @brief Initializes all of the semaphores that will be used by cc1120Send and
 * cc1120Receive
 *
 */
 ...
 *
 * @param numBytes - A pointer to an 8-bit integer to store the number of bytes
 * in
...
/**
 * @brief callback function to be used in an ISR when the TX FIFO drops below
 * (CC1120_TX_FIFO_SIZE - TXRX_INTERRUPT_THRESHOLD)
 */
void txFifoReadyCallback(void);

/**
 * @brief callback function to be used in an ISR when the RX FIFO fills above
 * TXRX_INTERRUPT_THRESHOLD
 */
void rxFifoReadyCallback(void);

/**
 * @brief callback function to be used in an ISR when the TX FIFO has become
 * empty
 */
void txFifoEmptyCallback(void);

/**
 * @brief callback function to be used in an ISR when the sync word has been
 * received
 */
