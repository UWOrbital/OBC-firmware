...
static register_setting_t cc1120SettingsStd[] = {
    ...
    // Set sync word qualifier value threshold similar to the one talked about
    // for preamble in section 6.8
...
obc_error_code_t cc1120Receive(void) {
  ...
  while (true) {
    ...
    // for more than RX_SEMAPHORE_TIMEOUT before exiting this loop since that
    // means we are no longer transmitting
