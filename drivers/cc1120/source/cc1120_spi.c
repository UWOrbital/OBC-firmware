 ...
 * @param data - The array to store the read data, or a pointer to a single
 * uint8_t if len=1.
 * @param len - The number of registers to read.
 * @return OBC_ERR_CODE_SUCCESS - If the read was successful.
 * @return An error code - If the register is not valid, or the errCode byte is
 * invalid.
 */
obc_error_code_t cc1120ReadSpi(uint8_t addr, uint8_t data[], uint8_t len) {
  ...
  uint8_t header =
      (len > 1) ? (READ_BIT | BURST_BIT | addr) : (READ_BIT | addr);
...
/**
 * @brief - Reads from consecutive extended address space registers on the
 * CC1120
 *
 * @param addr - The address of the first register to read.
 * @param data - The array to store the read data, or a pointer to a single
 ...
 * invalid.
 */
obc_error_code_t cc1120ReadExtAddrSpi(uint8_t addr, uint8_t data[],
                                      uint8_t len) {
  obc_error_code_t errCode;

  if (data == NULL) return OBC_ERR_CODE_INVALID_ARG;

  if ((addr >= CC1120_REGS_EXT_RESERVE_SPACE_1_START &&
       addr <= CC1120_REGS_EXT_RESERVE_SPACE_1_END))
    return OBC_ERR_CODE_INVALID_ARG;

  if ((addr >= CC1120_REGS_EXT_RESERVE_SPACE_2_START &&
       addr <= CC1120_REGS_EXT_RESERVE_SPACE_2_END))
    return OBC_ERR_CODE_INVALID_ARG;
  ...
  if (recvData != 0x00) {  // When sending the extended address, SO will return
                           // all zeros. See section 3.2.
 ...
 * @param data - The array of data to write to the registers, or a pointer to a
 * single uint8_t if len=1.
...
/**
 * @brief - Writes to consecutive extended address space registers on the
 * CC1120.
 *
 * @param addr - The address of the first register to write to.
 * @param data - The array of data to write to the registers, or a pointer to a
 ...
 * invalid.
 */
obc_error_code_t cc1120WriteExtAddrSpi(uint8_t addr, uint8_t data[],
                                       uint8_t len) {
  obc_error_code_t errCode;
  ...
  uint8_t header =
      (len > 1) ? (BURST_BIT | CC1120_REGS_EXT_ADDR) : CC1120_REGS_EXT_ADDR;
...
obc_error_code_t cc1120ReadFifo(uint8_t data[], uint8_t len) {
  ...
  uint8_t header = (len > 1)
                       ? (READ_BIT | BURST_BIT | CC1120_REGS_FIFO_ACCESS_STD)
 ...
 * invalid.
 */
obc_error_code_t cc1120ReadFifoDirect(uint8_t addr, uint8_t data[],
                                      uint8_t len) {
  obc_error_code_t errCode;
  ...
  uint8_t header = (len > 1)
                       ? (READ_BIT | BURST_BIT | CC1120_REGS_FIFO_ACCESS_DIR)
 ...
 *
 * @param addr - The address of the first register to write to. Range 0x00 -
 * 0xFF.
 * @param data - The array of data to write to the registers, or a pointer to a
 * single uint8_t if len=1.
 * @param len - The number of registers to write.
 ...
 * invalid.
 */
obc_error_code_t cc1120WriteFifoDirect(uint8_t addr, uint8_t data[],
                                       uint8_t len) {
...
/**
 * @brief - Reads the status register on the CC1120 and consecutively sends a
 * byte over SPI.
 *
 * @param data - The data to send to the status register.
 * @return OBC_ERR_CODE_SUCCESS - If the status byte is valid.
 * @return OBC_ERR_CODE_CC1120_INVALID_STATUS_BYTE - If the status byte is
 * invalid.
 */
obc_error_code_t cc1120SendByteReceiveStatus(uint8_t data) {
  ...
  // TODO: This is a hacky way to do this. We should implement a mutex +
  // timeout.
