...
void uartAssertFailed(char *file, int line, char *expr) {
  if (file == NULL || line < 0 || expr == NULL)
    return;  // Only called by assert, so we can assume that the arguments are
             // valid
...
obc_error_code_t sciReadByte(unsigned char *character) {
    SemaphoreHandle_t mutex = (UART_READ_REG == sciREG) ? sciMutex :
sciLinMutex; configASSERT(mutex != NULL);
    ...
    if (xSemaphoreTake(mutex, UART_MUTEX_BLOCK_TIME) == pdTRUE) {
        *character = (unsigned char)sciReceiveByte(UART_READ_REG); //
sciReceiveByte applies a 0xFF mask xSemaphoreGive(mutex); return
...
sciLinMutex; configASSERT(mutex != NULL);
    ...
    if (xSemaphoreTake(mutex, UART_MUTEX_BLOCK_TIME) == pdTRUE) {
        while(1) {
            cChar = (unsigned char) sciReceiveByte(UART_READ_REG); //
sciReceiveByte applies a 0xFF mask
