...
void osHandlerLM75BD(uint8_t devAddr) {
  if (devAddr == LM75BD_OBC_I2C_ADDR) {
    /*
        ...
        In OS comparator mode, the OS output will be deactivated automatically
       if T < Thys. In OS interrupt mode, the OS output will be deactivated only
       if we read from a register. Note that we only have to deal with the OS
       interrupt in OS interrupt mode.
     */
