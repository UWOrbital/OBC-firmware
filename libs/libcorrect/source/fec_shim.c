...
void *init_rs_char(int symbol_size, int primitive_polynomial,
    ...
    reed_solomon_shim *shim = sysMalloc(sizeof(reed_solomon_shim));
    ...
    shim->msg_out = sysMalloc(shim->block_length);
    shim->erasures = sysMalloc(number_roots);
...
static void *create_viterbi(unsigned int num_decoded_bits, unsigned int rate,
    ...
    convolutional_shim *shim = sysMalloc(sizeof(convolutional_shim));
    ...
    shim->rate = rate;
    shim->order = order;
    shim->buf = sysMalloc(num_decoded_bytes);
