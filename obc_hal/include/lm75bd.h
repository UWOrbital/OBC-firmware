...
typedef struct {
    ...
    uint8_t devOperationMode; // 0 = normal mode, 1 = shutdown/low-power mode
...
uint8_t readConfigLM75BD(uint8_t devAddr, lm75bd_config_t *config);

uint8_t readThystLM75BD(uint8_t devAddr, float *thyst);

uint8_t writeThystLM75BD(uint8_t devAddr, float thyst);

uint8_t readTosLM75BD(uint8_t devAddr, float *tos);

uint8_t writeTosLM75BD(uint8_t devAddr, float tos);

static uint8_t faultQueueSizeToRegData(uint8_t val);
