...
TEST(TestINA230, InitSuccess) {
  EXPECT_EQ(initINA230(), OBC_ERR_CODE_SUCCESS);
}
...
TEST(TestINA230, Current_I2CAddress) {
  float power = 0;
  EXPECT_EQ(getINA230Current(INA230_I2C_ADDRESS_TWO, &power), OBC_ERR_CODE_SUCCESS);
  EXPECT_EQ(getINA230Current(INA230_DEVICE_COUNT, &power), OBC_ERR_CODE_INVALID_ARG);
  EXPECT_EQ(getINA230CurrentForDevice(5, &power), OBC_ERR_CODE_INVALID_ARG);
}
