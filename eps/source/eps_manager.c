...
static uint8_t
    epsQueueStack[EPS_MANAGER_QUEUE_LENGTH * EPS_MANAGER_QUEUE_ITEM_SIZE];
...
void initEPSManager(void) {
  ...
  if (epsTaskHandle == NULL) {
    epsTaskHandle = xTaskCreateStatic(
        vEPSManagerTask, EPS_MANAGER_NAME, EPS_MANAGER_STACK_SIZE, NULL,
        EPS_MANAGER_PRIORITY, epsTaskStack, &epsTaskBuffer);
  }

  ASSERT((epsQueueStack != NULL) && (&epsQueue != NULL));
  if (epsQueueHandle == NULL) {
    epsQueueHandle = xQueueCreateStatic(EPS_MANAGER_QUEUE_LENGTH,
                                        EPS_MANAGER_QUEUE_ITEM_SIZE,
                                        epsQueueStack, &epsQueue);
  }
}
...
obc_error_code_t sendToEPSQueue(eps_event_t *event) {
  ...
  if (xQueueSend(epsQueueHandle, (void *)event,
                 EPS_MANAGER_QUEUE_TX_WAIT_PERIOD) == pdPASS)
...
static void vEPSManagerTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(epsQueueHandle, &queueMsg,
                      EPS_MANAGER_QUEUE_RX_WAIT_PERIOD) != pdPASS)
