...
static uint8_t
    adcsQueueStack[ADCS_MANAGER_QUEUE_LENGTH * ADCS_MANAGER_QUEUE_ITEM_SIZE];
...
void initADCSManager(void) {
  ...
  if (adcsTaskHandle == NULL) {
    adcsTaskHandle = xTaskCreateStatic(
        vADCSManagerTask, ADCS_MANAGER_NAME, ADCS_MANAGER_STACK_SIZE, NULL,
        ADCS_MANAGER_PRIORITY, adcsTaskStack, &adcsTaskBuffer);
  }

  ASSERT((adcsQueueStack != NULL) && (&adcsQueue != NULL));
  if (adcsQueueHandle == NULL) {
    adcsQueueHandle = xQueueCreateStatic(ADCS_MANAGER_QUEUE_LENGTH,
                                         ADCS_MANAGER_QUEUE_ITEM_SIZE,
                                         adcsQueueStack, &adcsQueue);
  }
}
...
obc_error_code_t sendToADCSQueue(adcs_event_t *event) {
  ...
  if (xQueueSend(adcsQueueHandle, (void *)event,
                 ADCS_MANAGER_QUEUE_TX_WAIT_PERIOD) == pdPASS)
...
static void vADCSManagerTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(adcsQueueHandle, &queueMsg,
                      ADCS_MANAGER_QUEUE_RX_WAIT_PERIOD) != pdPASS)
