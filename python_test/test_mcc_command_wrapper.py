...
async def test_create_main_command(db_session):
    data = {"id": 1, "name": "Test Command", "params": None, "format": None, "data_size": 4, "total_size": 4}
    command = await wrapper.create_main_command(data)
...
async def test_get_all_main_commands(db_session):
    ...
    commands = await wrapper.get_all_main_commands()
...
async def test_delete_main_command_by_id(db_session):
    # Insert one command
    db_session.add(MainCommand(id=1, name="CmdX", params=None, format=None, data_size=1, total_size=1))
    db_session.commit()

    # Should delete successfully
    result = await wrapper.delete_main_command_by_id(1)
