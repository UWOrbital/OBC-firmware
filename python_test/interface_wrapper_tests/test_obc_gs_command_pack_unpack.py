import pytest
from interfaces.obc_gs_interface.python_wrappers.pack_unpack.obc_gs_pack_unpack import packCmdMsg, unpackCmdMsg, CMD_END_OF_FRAME, CMD_RTC_SYNC
...
def test_pack_cmd_valid_data(valid_command_data, create_command_buffer):
    ...
    result = packCmdMsg(buffer, offset, cmd_data, num_packed)

    assert result == obc_gs_errors.OBC_GS_ERR_CODE_SUCCESS, "Packing command data failed with valid data"

@pytest.mark.parametrize("cmd_data, buffer, expected_error", [
    (None, bytearray(256), obc_gs_errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid cmd_data
    ({"id": CMD_END_OF_FRAME, "timestamp": 1609459200, "isTimeTagged": True}, bytearray(256), obc_gs_errors.OBC_GS_ERR_CODE_UNSUPPORTED_CMD),  # Unsupported command ID
    ({"id": CMD_END_OF_FRAME, "timestamp": 1609459200, "isTimeTagged": True}, None, obc_gs_errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid buffer
])
...
@pytest.mark.parametrize("buffer, offset, cmd_data, expected_error", [
    (None, 0, {"id": 2, "timestamp": 1609459200, "isTimeTagged": True}, obc_gs_errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid buffer
    (bytes(256), None, {"id": 2, "timestamp": 1609459200, "isTimeTagged": True}, obc_gs_errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid offset
...
def test_unpack_cmd_invalid_args(buffer, offset, cmd_data, expected_error):
    result = unpackCmdMsg(buffer, offset, cmd_data)
    assert result == expected_error, "Incorrect error returned"
