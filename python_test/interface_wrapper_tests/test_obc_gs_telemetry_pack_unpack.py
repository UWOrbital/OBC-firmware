...
@pytest.mark.parametrize("telemetry_data, buffer, buffer_len, expected_error", [
    ...
    ({"id": pack_unpack.TELEM_NONE, "timestamp": 1609459200}, bytearray(256), 256, errors.OBC_GS_ERR_CODE_INVALID_ARG), # ID is 0
    ({"id": pack_unpack.TELEM_CC1120_TEMP, "timestamp": 1609459200}, bytearray(256), 256, errors.OBC_GS_ERR_CODE_INVALID_ARG), # No pack function
    ({"id": pack_unpack.TELEM_OBC_TEMP, "timestamp": 1609459200}, None, 256, errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid buffer
    ({"id": pack_unpack.TELEM_OBC_TEMP, "timestamp": 1609459200, "data": {"obcTemp": 45.5}}, bytearray(256), 0, errors.OBC_GS_ERR_CODE_BUFF_TOO_SMALL),  # Buffer too small
])
...
@pytest.mark.parametrize("buffer, offset, telemetry_data, expected_error", [
    (None, 0, {"id": pack_unpack.TELEM_OBC_TEMP, "timestamp": 1609459200}, errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid buffer
    (bytes(256), None, {"id": pack_unpack.TELEM_OBC_TEMP, "timestamp": 1609459200}, errors.OBC_GS_ERR_CODE_INVALID_ARG),  # Invalid offset
