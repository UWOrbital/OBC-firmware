...
static unsigned int reed_solomon_find_error_locator(correct_reed_solomon *rs, size_t num_erasures) {
    ...
    for (unsigned int i = rs->error_locator.order; i < rs->min_distance - num_erasures; i++) {
        ...
        if (2 * numerrors <= i) {
            ...
            for (uint8_t j = 0; j <= (rs->last_error_locator.order + delay_length); j++) {
...
void correct_reed_solomon_decoder_create(correct_reed_solomon *rs) {
    ...
    rs->syndromes = obcMalloc(rs->min_distance*sizeof(field_element_t));
    memset(rs->syndromes, 0, rs->min_distance*sizeof(field_element_t));
    rs->modified_syndromes = obcMalloc(2 * rs->min_distance * sizeof(field_element_t));
    ...
    rs->error_roots = obcMalloc(2 * rs->min_distance * sizeof(field_element_t));
    memset(rs->error_roots, 0, 2 * rs->min_distance * sizeof(field_element_t));
    rs->error_vals = obcMalloc(rs->min_distance * sizeof(field_element_t));
    rs->error_locations = obcMalloc(rs->min_distance * sizeof(field_logarithm_t));
    ...
    rs->generator_root_exp = obcMalloc(rs->min_distance * sizeof(field_logarithm_t *));
    for (unsigned int i = 0; i < rs->min_distance; i++) {
        rs->generator_root_exp[i] = obcMalloc(rs->block_length * sizeof(field_logarithm_t));
    ...
    rs->element_exp = obcMalloc(256 * sizeof(field_logarithm_t *));
    for (field_operation_t i = 0; i < 256; i++) {
        rs->element_exp[i] = obcMalloc(rs->min_distance * sizeof(field_logarithm_t));
...
ssize_t correct_reed_solomon_decode_with_erasures(correct_reed_solomon *rs, const uint8_t *encoded,
    ...
    field_element_t *syndrome_copy = obcMalloc(rs->min_distance * sizeof(field_element_t));
