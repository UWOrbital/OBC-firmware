...
obc_error_code_t readNextTelemetryFromFile(int32_t telemFileId, telemetry_data_t *telemData) {
  ...
  // We're only sending X past states per downlink, so we should only need to
  ...
  // Since we only write the telemetry data struct, the number of bytes in the
  // file should be a multiple of the size of the struct.
