...
static void timekeeperTask(void *pvParameters) {
  /*
   * The timekeeper is a task instead of a FreeRTOS timer because we have more
   * control over its priority relative to other tasks (inc. the time service
   * daemon). The sync period is often enough (and the MCU clock speed is high
   * enough) that we don't need to worry about significant drift.
   *
   * The I2C clock speed is high enough that, if another task is completing an
   * I2C transaction when the timekeeper task is performing a local RTC sync,
   * the timekeeper task should not have to wait a significant amount of time.
   */
  ...
  while (1) {
    if (syncPeriodCounter == 0) {
      // Sync the local time with the RTC every LOCAL_TIME_SYNC_PERIOD_S
      // seconds.
