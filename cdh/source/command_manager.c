...
/**
 * @brief Executes the command callback associated with the given command
 * 
 * @param cmd Command to execute
 * @return obc_error_code_t OBC_ERR_CODE_SUCCESS if the command was executed successfully, otherwise an error code
 */
static obc_error_code_t execCommandCallback(cmd_msg_t *cmd);


void initCommandManager(void) {
    ASSERT( (cmdManagerTaskStack != NULL) && (&cmdManagerTaskBuffer != NULL) );
    if (cmdManagerTaskHandle == NULL) {
        cmdManagerTaskHandle = xTaskCreateStatic(commandManagerTask, CMD_MANAGER_TASK_NAME, CMD_MANAGER_TASK_STACK_SIZE, NULL, CMD_MANAGER_TASK_PRIORITY, cmdManagerTaskStack, &cmdManagerTaskBuffer);
    }

    ASSERT( (commandQueueStack != NULL) && (&commandQueue != NULL) );
    if (commandQueueHandle == NULL) {
        commandQueueHandle = xQueueCreateStatic(COMMAND_QUEUE_LENGTH, COMMAND_QUEUE_ITEM_SIZE, commandQueueStack, &commandQueue);
    }
}
...
static obc_error_code_t execCommandCallback(cmd_msg_t *cmd) {
    ...
    switch(cmd->id) {
        case CMD_EXEC_OBC_RESET:
            return execObcResetCmdCallback();

        case CMD_RTC_SYNC:
            return rtcSyncCmdCallback(cmd->rtcSync);
        
        case CMD_DOWNLINK_LOGS_NEXT_PASS:
            return downlinkLogsNextPassCmdCallback(cmd->downlinkLogsNextPass);
