...
ax25_addr_t cubesatCallsign = {
    .data = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
    .length = AX25_DEST_ADDR_BYTES};  // mock cubesat address

ax25_addr_t groundStationCallsign = {
    .data = {0},
    .length = AX25_DEST_ADDR_BYTES};  // Mock Ground station address

/**
 * @brief performs bit unstuffing on a receive ax.25 packet
 *
 * @param packet - pointer to a received stuffed ax.25 packet
 * @param unstuffedPacket - pointer to a unstuffed_ax25_packet_t struct to hold
 * the unstuffed ax.25 packet
 *
 * @return obc_error_code_t OBC_ERR_CODE_SUCCESS if it was successful and error
 * code if not
 */
static obc_error_code_t ax25Unstuff(const packed_ax25_packet_t *packet,
 ...
 * @param rsData 255 byte array to store the reed solomon encoded data without
 * ax.25 headers
 ...
 * code if not
 */
static obc_error_code_t sFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket,
 ...
 * code if not
 */
static obc_error_code_t iFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket,
                                   packed_rs_packet_t *rsData,
                                   ax25_addr_t *recvAddress);
 ...
 * code if not
 */
static obc_error_code_t fcsCalculate(const uint8_t *data,
                                     uint16_t *calculatedFcs);
 ...
 *
 * @return obc_error_code_t OBC_ERR_CODE_SUCCESS if it was a valid fcs and error
 * code if not
 */
 ...
 * code if not
 */
static obc_error_code_t bitStuffing(uint8_t *rawData,
                                    packed_ax25_packet_t *stuffedData);

/**
 * @brief adds ax.25 headers onto telemetry being downlinked and stores the
 * length of the packet in az25Data->length
 *
...
obc_error_code_t ax25Send(packed_rs_packet_t *rsData,
  ...
  memcpy(ax25PacketUnstuffed + AX25_START_FLAG_BYTES, destAddress->data,
         AX25_DEST_ADDR_BYTES);
  memcpy(ax25PacketUnstuffed + AX25_START_FLAG_BYTES + AX25_DEST_ADDR_BYTES,
         srcAddress->data, AX25_SRC_ADDR_BYTES);
  ax25PacketUnstuffed[AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES] =
      (pktReceiveNum << 1);
  ax25PacketUnstuffed[AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES + 1] =
      (pktSentNum << 1);
  ax25PacketUnstuffed[AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES +
                      AX25_CONTROL_BYTES] = AX25_PID;
  memcpy(ax25PacketUnstuffed + AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES +
             AX25_CONTROL_BYTES + AX25_PID_BYTES,
         rsData->data, RS_ENCODED_SIZE);
  uint16_t fcs;
  RETURN_IF_ERROR_CODE(fcsCalculate(ax25PacketUnstuffed, &fcs));
  ax25PacketUnstuffed[AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES +
                      AX25_CONTROL_BYTES + AX25_PID_BYTES + AX25_INFO_BYTES] =
      (uint8_t)(fcs >> 8);
  ax25PacketUnstuffed[AX25_START_FLAG_BYTES + AX25_ADDRESS_BYTES +
                      AX25_CONTROL_BYTES + AX25_PID_BYTES + AX25_INFO_BYTES +
                      1] = (uint8_t)(fcs & 0xFF);
...
obc_error_code_t ax25Recv(packed_ax25_packet_t *ax25Data,
  ...
  uint16_t fcs =
      unstuffedPacket
          .data[AX25_INFO_BYTES + AX25_PID_BYTES + AX25_CONTROL_BYTES +
                AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES]
      << 8;
  fcs |= unstuffedPacket
             .data[AX25_INFO_BYTES + AX25_PID_BYTES + AX25_CONTROL_BYTES +
                   AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES + 1];
...
static obc_error_code_t ax25Unstuff(const packed_ax25_packet_t *packet,
  ...
  // flags
  for (uint16_t stuffedPacketIndex = 1; stuffedPacketIndex < packet->length - 1;
       ++stuffedPacketIndex) {
    ...
    for (uint8_t offset = 0; offset < 8; ++offset) {
      ...
      unstuffedPacket->data[unstuffedBitLength / 8] |=
          bit << (7 - (unstuffedBitLength % 8));
      unstuffedBitLength++;
    }
  }
...
static obc_error_code_t sFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket,
                                   packed_rs_packet_t *rsData,
                                   ax25_addr_t *recvAddress) {
  if (memcmp(unstuffedPacket->data + AX25_START_FLAG_BYTES, recvAddress->data,
             AX25_DEST_ADDR_BYTES) != 0) {
    return OBC_ERR_CODE_INVALID_AX25_PACKET;
  }
  uint8_t controlBytes[AX25_CONTROL_BYTES] = {
...
static obc_error_code_t iFrameRecv(unstuffed_ax25_packet_t *unstuffedPacket,
  ...
  if (unstuffedPacket->data[AX25_CONTROL_BYTES + AX25_ADDRESS_BYTES + 1] !=
      AX25_PID) {
    return OBC_ERR_CODE_INVALID_AX25_PACKET;
  }
  memcpy(rsData->data,
         unstuffedPacket->data + AX25_PID_BYTES + AX25_CONTROL_BYTES +
             AX25_ADDRESS_BYTES + AX25_START_FLAG_BYTES,
         AX25_INFO_BYTES);
  pktReceiveNum++;
  return OBC_ERR_CODE_SUCCESS;
...
static obc_error_code_t fcsCalculate(const uint8_t *data,
  ...
  *calculatedFcs = 0xFFFF;  // Initial calculatedFcs value
  ...
  for (uint16_t i = 0;
       i < (AX25_MINIMUM_I_FRAME_LEN - AX25_FCS_BYTES - AX25_END_FLAG_BYTES);
       ++i) {
    *calculatedFcs ^= (uint16_t)data[i] << 8;
    ...
    for (uint8_t j = 0; j < 8; ++j) {
      if (*calculatedFcs & 0x8000) {
        *calculatedFcs =
            (*calculatedFcs << 1) ^ 0x8408;  // Polynomial X^16 + X^12 + X^5 + 1
      } else {
        *calculatedFcs <<= 1;
      }
    }
...
static obc_error_code_t bitStuffing(uint8_t *rawData,
  ...
  for (rawOffset = 8; rawOffset < (AX25_MINIMUM_I_FRAME_LEN - 1) * 8;
       ++rawOffset) {
    currentBit = (rawData[rawOffset / 8] >> (7 - (rawOffset % 8))) & 1;
    stuffedData->data[stuffedOffset / 8] |=
        (currentBit << (7 - (stuffedOffset % 8)));
    stuffedOffset++;
